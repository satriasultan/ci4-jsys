                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            value="<?= !empty($tglawal) ? date('d-m-Y', strtotime($tglawal)) : '' ?>">
                        </div>
                        <button type="submit"
                                onclick="processTglAwal()"
                                class="btn btn-primary btn-lg float-right">
                            <i class="fa fa-save"></i> Proses Tanggal Awal
                        </button>
                    </div>
                </div>
			</div><!-- /.card-body -->
...
<script type="application/javascript" src="<?= base_url('assets/pagejs/tools/sta.js') ?>"></script>
<script type="text/javascript">
    $(function() {
        ...
        $('#tglawal').on('apply.daterangepicker', function(ev, picker) {
            $(this).val(picker.startDate.format('DD-MM-YYYY'));
            // jika butuh validasi bootstrapValidator:
            // $('#formInputTransfers').bootstrapValidator('updateStatus', 'tglawal', 'NOT_VALIDATED').bootstrapValidator('validateField', 'tglawal');
        });
        $('#tglawal').on('cancel.daterangepicker', function(ev, picker) {
            $(this).val('');
        });
