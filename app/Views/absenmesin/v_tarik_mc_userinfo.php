...
<script>
    $(function () {
        $('#qintil').click(function () {
            ...
            $.ajax({
                url: "<?php echo site_url('absenmesin/absenmesin/save_mc_userinfo')?>",
                data: formData,
                processData: false,
                contentType: false,
                type: 'POST',
                               ...
                               // var time = parseInt(xhr.readyState);
                                ...
                                var interval = setInterval(function() {
                                    if (time <= 10) {
                                        ...
                                        console.log(xhr.responseText.trim() == 'TRUE');
                                        //time ++;
                                        if ((time <= 9 && xhr.status != 200) || (xhr.responseText.trim() == 'FALSE')) {
                                            console.log(parseInt(xhr.status));
                                            if (xhr.status == 500 ||  xhr.responseText != 'FALSE') {
                                            ...
                                            if (this.status == 200 || xhr.responseText.trim() == 'TRUE') {
                                                time = 10;
