...
class Lembur extends BaseController
{
    ...
    function hps_lembur($nodok){
        ...
        /*if ($cek>0 or $cek2>0) {
            ...
            $this->db->where('nodok',$nodok);
            $this->db->update('sc_trx.lembur',$info);
            return redirect()->to(base_url("trans/lembur/index/del_succes");
        }*/


        if ($tgl_closing>=$tgl_dok){
            return redirect()->to(base_url("trans/lembur/index/del_failed"));
        } else {
            $this->db->where('nodok',$nodok);
            $this->db->update('sc_trx.lembur',$info);
            return redirect()->to(base_url("trans/lembur/index/del_succes"));
    ...
    function proses_xls_lembur() {
        ...
        if (!$validateFile){
            ...
            for ($row = 2; $row <= $highestRow; $row++){  				//  Read a row of data into an array
                ...
                $cek = $this->m_lembur->q_lembur($parametercek)->getNumRows();
                if (!empty($rowData[0][0]) and $cek == 0) {
