...
class Candidateos extends BaseController
{
    public function index()
    {
        $data['title']='Data Karyawan Outsource';

        /* CODE UNTUK VERSI*/
        $nama=trim($this->session->get('nama'));
        $kodemenu='I.T.A.2'; $versirelease='I.T.A.2/RELEASE.401'; $releasedate=date('2019-04-12 00:00:00');
        ...
        return $this->template->render('trans/candidateos/v_karyawan',$data);
    }
    ...
    public function ajax_list()
    {
        ...
        $list = $this->m_candidateos->get_datatables();
        $data = array();
        $no = $_POST['start'];
        $nama=trim($this->session->get('nama'));
        foreach ($list as $person) {
            ...
            $row = array();
            $row[] = $no;
            if ((trim($person->status)==='I' and $role !== 'AHC') or (trim($person->status)==='I' and $role !== 'ML')) {
                $row[] = '
<div class="dropdown">
    ...
    <ul class="dropdown-menu" role="menu" aria-labelledby="menu1" style="background: #b3efff">
        ...
        <li role="presentation"><a role="menuitem" href="'.base_url('trans/candidateos/ajax_delete').'/'.trim($person->nik).'"  onclick="return confirm('."'Apakah Anda Yakin Cancel Ini?  ".trim($person->nik)."'".')"><i class="fa fa-close"></i> Cancel </a></li>
        <!--li role="presentation"><a role="menuitem" href="#" title="Detail Atribut Karyawan"  onclick="detailAtributNik('."'".trim($person->nik)."'".');"><i class="fa fa-dashcube"></i> Upload Atribut Pendukung</a></li-->
        <li role="presentation"><a role="menuitem" href="'.base_url('trans/candidateos/send_datahc').'/'.trim($person->nik).'" title="Send Data" onclick="return confirm('."'Data Karyawani Ini akan diproses, pastikan data benar, karena setelah proses ini , edit tidak diperkenankan, anda yakin?  ".trim($person->nik)."'".')"><i class="fa fa-send"></i> Send Data </a></li>
    </ul>
</div>
               ';
            } else if ((trim($person->status)==='A' and $role === 'AHC') or (trim($person->status)==='A' and $role === 'ML')) {
                $row[] = '
<div class="dropdown">
    <button class="btn btn-primary dropdown-toggle " style="margin:0px; color:#FFFFFF;" id="menu1" type="button" data-bs-toggle="dropdown" autocomplete="off" aria-expanded="false"><i class="fa fa-gear"></i>
        <span class="caret"></span></button>
...
<div class="dropdown">
    ...
    <ul class="dropdown-menu" role="menu" aria-labelledby="menu1" style="background: #b3efff">
        <li role="presentation"><a role="menuitem" title="Detail" href="#" onclick="detailNik('."'".trim($person->nik)."'".');"><i class="fa fa-bars"></i> Detail </a></li>
            ...
            if ($person->status==='A'){
                $row[] = '<span class="label label-primary" style="font-size: 100%">'.$person->nmstatus.'</span>';
            } else if ($person->status==='P') {
                $row[] = '<span class="label label-success" style="font-size: 100%">'.$person->nmstatus.'</span>';
            } else if ($person->status==='C') {
                $row[] = '<span class="label label-danger" style="font-size: 100%">'.$person->nmstatus.'</span>';
            } else {
                $row[] = '<span class="label label-default" style="font-size: 100%">'.$person->nmstatus.'</span>';
            }

            $row[] = $person->nikupdated;
            $row[] = $person->inputby;
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->m_candidateos->count_all(),
            "recordsFiltered" => $this->m_candidateos->count_filtered(),
            "data" => $data,
        );
    ...
    public function input(){
        ...
        $data['title'] = "Input Karyawan Outsource";
        $data['type']="INPUT";
        $data['nama']=$nama;
        return $this->template->render('trans/candidateos/v_input_karyawan',$data);
    }
    ...
    function showDetailNik () {
        ...
        $param = " and nik='$docno'";
        $dtl = $this->m_candidateos->list_karyawan_param($param);
    ...
    function saveDataMasterKaryawan(){
        ...
        $nikupdated = strtoupper(trim($this->request->getPost('nikupdated')));
        ...
        $builder_t_karyawan = $this->db->table('sc_mst.t_karyawan_candidate');
        ...
        if (!empty($file->getName())) {
            ...
            if ($type==='INPUT'){
                ...
                if ($builder_t_karyawan->insert($info)) {
                    ...
                    $builder_trxerror->where('modul','NIK-CANDIDATE');
                    ...
                    $paramerror=" and userid='$nama' and modul='NIK-CANDIDATE'";
                ...
                if ($this->db->update('sc_mst.t_karyawan_candidate', $infoX)) {
                    $paramerror=" and userid='$nama' and modul='NIK-CANDIDATE'";
                    $dtlerror=$this->m_trxerror->q_trxerror($paramerror)->getRowArray();
                    $getResult = array('status' => true, 'messages' => 'Sukses Di Proses'.' Nomor: '.$nik);
                    echo json_encode($getResult);
                } else {
                    $getResult = array('status' => false, 'messages' => 'Data Gagal Di Proses Ada Kesalahan Data 2');
                    echo json_encode($getResult);
    ...
    {
        $data = $this->m_candidateos->get_by_id($id)->row();
    ...
    public function detail($id)
    {
        ...
        $data['list_opt_atasan']=$this->m_candidateos->list_karyawan()->getResult();
        ...
        $data['list_finger']=$this->m_candidateos->q_finger()->getResult();//
        $data['list_kanwil']=$this->m_candidateos->q_kanwil()->getResult();//
        ...
        $data['title'] = "Detail Karyawan";
        //$data['dtl'] = $this->m_candidateos->get_dtl_id($id)->getRowArray();
        $data['lp'] = $this->m_candidateos->get_dtl_id($id)->getRowArray();
        return $this->template->render('trans/candidateos/v_detailhrdkary',$data);
    }
    ...
    public function detail_resign($id)
    {
        ...
        $data['list_finger']=$this->m_candidateos->q_finger()->getResult();//
        $data['list_kanwil']=$this->m_candidateos->q_kanwil()->getResult();//
        /// $data['list_wilnom']=$this->m_candidateos->q_wilayah_nominal($p=null)->getResult();// wilayah nominal
        ...
        //$data['dtl'] = $this->m_candidateos->get_dtl_id($id)->getRowArray();
        $data['lp'] = $this->m_candidateos->get_dtl_id($id)->getRowArray();
        return $this->template->render('trans/candidateos/v_detailhrdkary_resign',$data);
    }
    ...
    public function edit($id){
        ...
        $data['list_kanwil']=$this->m_candidateos->q_kanwil()->getResult();//
        $data['title'] = "Edit Karyawan";
        $data['dtl'] = $this->m_candidateos->get_by_id($id)->getRowArray();
        return $this->template->render('trans/candidateos/v_editkary',$data);
    }


    public function ajax_delete($id)
    {
        //$this->m_candidateos->delete_by_id($id);
        //echo json_encode(array("status" => TRUE));
        $nama = trim($this->session->get('nama'));
        $builder = $this->db->table('sc_mst.t_karyawan_candidate');
        $builder_trxerror = $this->db->table('sc_mst.trxerror');
        $info = array('status' => 'C');
        ...
        $builder_trxerror->insert($infotrxerror);
        return redirect()->to(base_url('trans/candidateos'));
    }

    public function send_datahc($id)
    {
        //pake nunut description
        $nama = trim($this->session->get('nama'));
        //$this->m_candidateos->delete_by_id($id);
        //echo json_encode(array("status" => TRUE));
        $builder = $this->db->table('sc_mst.t_karyawan_candidate');
    ...
    public function hc_approved($id)
    {
        //pake nunut description
        $nama = trim($this->session->get('nama'));
        $builder = $this->db->table('sc_mst.t_karyawan_candidate');
        ...
        $builder_trxerror->insert($infotrxerror);
         return redirect()->to(base_url('trans/candidateos/approvalpage'));
    }
    public function hc_sendback($id)
    {
        //pake nunut description
        $nama = trim($this->session->get('nama'));
        $builder = $this->db->table('sc_mst.t_karyawan_candidate');
    ...
    public function excel_listkaryawan(){
        ...
        $datane = $this->m_candidateos->list_karyawan();
        $this->fiky_excel->set_query($datane);
        $this->fiky_excel->set_header(array(
            'NIK','NAMA LENGKAP','PANGGILAN','JENIS KELAMIN','WARGA NEGARA','TANGGAL LAHIR','ALAMAT LAHIR','NO KTP',
            'ALAMAT KTP','ALAMAT DOMISILI','STATUS KAWIN','NPWP','STATUSPTKP','TGL MASUKKERJA','KEPEGAWAIAN','KANTOR','PLANT',
            'DIVISI','DEPARTMEN','SECTION','POSITION','GRADE','GROUP','COSTCENTER','KK','HP1','HP2',
            'EMAIL OFFICE','EMAIL PRIBADI','BANK','NO REKENING','JENJANG','JENJANG DI','JURUSAN','TAHUN LULUS','IPK','NAMA IBU KANDUNG',
            'ID BPJSKES','CABANG BPJSKES','ID BPJSNAKER','CAB BPJS NAKER','ATASAN1','ATASAN2'
        ));



        $this->fiky_excel->set_column(array(
            'nik1','nmlengkap','callname','jk','stswn','tgllahir1','alamatlahir','noktp',
            'desc_ktp','desc_domisili','sts_kawin','nonpwp','sts_ptkp','tglmasukkerja1','nmkepegawaian','type_office','locaname',
            'nmdivision','nmdept','nmsubdept','nmjabatan','nmgrade','nmgroupgol','nmcostcenter','idkk','hp1','hp2',
            'email_office','email_self','nmbank','norekening','nmpendidikan','pendidikan_terakhir','jurusan','lulus_tahun','ipk','nmibukandung',
            'id_bpjs_kes','cab_bpjs_kes','id_bpjs_naker','cab_bpjs_naker','nmatasan1','nmatasan2'
        ));

        $this->fiky_excel->set_width(array(
            10,20,20,20,20,20,20,50,
            20,20,20,50,20,20,20,20,20,
            20,20,20,20,20,20,20,50,30,30,
            20,20,20,50,20,20,20,20,20,20,
            20,20,20,20,20,20
        ));
    ...
    function up_foto(){
        ...
        $nm = $this->m_candidateos->get_by_id($nik)->getRowArray();
        $nama=$nm['nmlengkap'];
        ...
        if(!$this->upload->do_upload('gambar')){
            ...
            if ( ! $this->image_lib->resize()){
                ...
                //update foto pegawai
                $this->m_candidateos->save_foto($nik,$info);
            ...
            //echo "<img id='gbr' src='$x' width='100%' height='100%' alt='User Image'>";
             return redirect()->to(base_url("trans/candidateos/detail/$nik"));
        }
        //tampilkan pesan
    ...
    function simpanmutasi (){
        ...
        $this->db->insert('sc_tmp.mutasi',$info);
         return redirect()->to(base_url("trans/candidateos/detail/$nik/success"));
    ...
    function add_bpjs(){
        ...
        if ($cek>0){
             return redirect()->to(base_url("trans/candidateos/detail/$nik/kode_failed"));
        } else {
    ...
    function hps_bpjs($nik,$id_bpjs){
        ...
        $this->db->delete('sc_trx.bpjs_karyawan');
         return redirect()->to(base_url("trans/candidateos/detail/$nik/del_success"));
    }
    ...
    function karyawan_self(){
        ...
        $data['list_opt_atasan']=$this->m_candidateos->list_karyawan()->getResult();
        $data['list_opt_ptkp']=$this->m_bpjs->list_ptkp()->getResult();
        //$data['list_opt_ptkp']=$this->m_candidateos->list_ptkp()->getResult();
        $data['list_opt_grp_gaji']=$this->m_group_penggajian->q_group_penggajian()->getResult();
        $data['list_opt_bank']=$this->m_bank->q_bank()->getResult();
        $data['list_resignkary']=$this->m_candidateos->list_karyresgn()->getResult();//
        $data['list_borong']=$this->m_candidateos->list_karyborong()->getResult();//
        $data['calon_karyawan']=$this->m_calonkaryawan->q_maxktpcalon()->getResult();//
        $data['list_finger']=$this->m_candidateos->q_finger()->getResult();//
        $data['list_kanwil']=$this->m_candidateos->q_kanwil()->getResult();//
        $data['list_self']=$this->m_candidateos->get_dtl_id($id)->getResult();
        ...
        return $this->template->render('trans/candidateos/v_karyawan_self',$data);
    }
    ...
    function detail_self($id)
    {
        ...
        //$data['dtl'] = $this->m_candidateos->get_dtl_id($id)->getRowArray();
        $data['lp'] = $this->m_candidateos->get_dtl_id($id)->getRowArray();
        return $this->template->render('trans/candidateos/v_detailhrdkary_self',$data);
    }
    function edit_self($id)
    {
        ...
        $data['title'] = "Edit Karyawan";
        $data['dtl'] = $this->m_candidateos->get_by_id($id)->getRowArray();
        return $this->template->render('trans/candidateos/v_editkary_self',$data);
    }
    ...
    function update_self_person(){
        ...
        $this->db->update('sc_mst.karyawan',$info);
         return redirect()->to(base_url("trans/candidateos/karyawan_self/upsuccess"));
    }
    ...
    function add_riwayat_keluarga(){
        ...
        $inputby=$this->session->get('nama');
        if (empty($kdkeluarga)){
             return redirect()->to(base_url("trans/candidateos/index/"));
        }
        ...
        $this->db->insert('sc_trx.riwayat_keluarga',$info);
         return redirect()->to(base_url("trans/candidateos/detail/$nik/rep_succes"));
    ...
    function hps_riwayat_kesehatan($nik,$no_urut){
        ...
        $this->db->delete('sc_trx.riwayat_kesehatan');
         return redirect()->to(base_url("trans/candidateos/detail/$nik/del_succes"));
    }
    ...
    {
        $list = $this->m_candidateos->get_t_resign_view();
        $data = array();
        $no = $_POST['start'];
        $nama=trim($this->session->get('nama'));
        foreach ($list as $person) {
            ...
            $row[] = '<a class="btn btn-sm btn-default" href="#" title="Detail Karyawan"  onclick="detailNik('."'".trim($person->nik)."'".');"><i class="fa fa-bars"></i></a>
                        <a class="btn btn-sm btn-primary" href="#"  title="Ubah Karyawan" onclick="editNik('."'".trim($person->nik)."'".');"><i class="fa fa-gear"></i> </a>
						';
            $row[] = $person->nik;
            $row[] = $person->nmlengkap;
            /*
    ...
    public function detail_atribut_karyawan(){
        ...
        $data['docno'] =$docno;
        /* RIWAYAT PENDIDIKAN */
        $data['riwayat_pendidikan'] = $this->m_candidateos->q_riwayat_pendidikan(" and nik='$docno'")->getResult();
        $data['m_pendidikan'] = $this->m_candidateos->q_m_pendidikan(" ")->getResult();
        /* KARTU PENDUKUNG */
        $data['list_kartupendukung'] = $this->m_candidateos->q_kartupendukung(" and nik='$docno'")->getResult();
        $data['m_kartupendukung'] = $this->m_candidateos->q_m_kartupendukung(" ")->getResult();
        return $this->template->render('trans/candidateos/v_detail_atribut_karyawan',$data);
    ...
    function save_pendidikanf(){
        ...
        if (!empty($_FILES['pend_inp_attachment']['name'])) {
            ...
            $config['max_height'] = '3200';
            $this->upload->initialize($config);
            if (!$this->upload->do_upload('pend_inp_attachment')) {
                $getResult = array('status' => false, 'messages' => 'Gambar Tidak Sesuai</br>
					Format yang sesuai:</br>
					* Ukuran File yang di ijinkan max 2MB</br>
					* Lebar Max 250 pixel</br>
					* Tinggi Max 250 pixel</br>					
					');
                json_encode($getResult);
                //INSERT TRX ERROR
                $this->db->where('userid',$nama);
                $this->db->where('modul','I.T.A.2');
                ...
                $this->db->insert('sc_mst.trxerror',$infotrxerror);
                 return redirect()->to(base_url("trans/candidateos/detail_atribut_karyawan/?docno=$nik"));
            } else {
                //Image Resizing
                $config['source_image'] = $this->upload->upload_path . $this->upload->file_name;
                //$config['maintain_ratio'] = FALSE;
                ...
                if (!$this->image_lib->resize()) {
                    $getResult = array('status' => false, 'messages' => $this->image_lib->display_errors('', ''));
                    echo json_encode($getResult);
                } else {
                    $res = $this->upload->data();
                    $file_path = $res['file_path'];
                    $file = $res['full_path'];
                    $file_ext = $res['file_ext'];
                    $final_file_name = trim(trim($nik)) .'pen_f_'.$kdpendidikan. $file_ext;
                    if (file_exists($file_path . $final_file_name)) {
                        unlink($file_path . $final_file_name);
                    }
                    rename($file, $file_path . $final_file_name);
                ...
                $this->db->insert('sc_his.riwayat_pendidikan',$insert_this);
    ...
    function save_pendidikanf_update(){
        ...
        if (!empty($_FILES['pend_upd_attachment']['name'])) {
            $config['upload_path'] = $path;
            $config['allowed_types'] = 'gif|jpg|png|jpeg';
            $config['max_size'] = '5000';
            $config['max_width'] = '3624';
            $config['max_height'] = '3200';
            $this->upload->initialize($config);
            if (!$this->upload->do_upload('pend_upd_attachment')) {
                $getResult = array('status' => false, 'messages' => 'Gambar Tidak Sesuai</br>
					Format yang sesuai:</br>
					* Ukuran File yang di ijinkan max 2MB</br>
					* Lebar Max 250 pixel</br>
                ...
                $this->db->where('id',$id);
                $this->db->update('sc_his.riwayat_pendidikan',$insert_this);
    ...
    function save_pendidikanf_delete(){
        ...
        $dtl = $this->m_candidateos->q_riwayat_pendidikan(" and id='$id'")->getRowArray();
        ...
        $this->db->where('id',$id);
        $this->db->delete('sc_his.riwayat_pendidikan');
    ...
    function save_kartupendukung(){
        ...
        if (!empty($_FILES['kartupendukung_inp_attachment']['name'])) {
            $config['upload_path'] = $path;
            $config['allowed_types'] = 'gif|jpg|png|jpeg';
            $config['max_size'] = '5000';
            $config['max_width'] = '3624';
            $config['max_height'] = '3200';
            $this->upload->initialize($config);
            if (!$this->upload->do_upload('kartupendukung_inp_attachment')) {
                $getResult = array('status' => false, 'messages' => 'Gambar Tidak Sesuai</br>
					Format yang sesuai: * Ukuran File yang di ijinkan max 2MB</br>');
                json_encode($getResult);
                //INSERT TRX ERROR
                $this->db->where('userid',$nama);
                $this->db->where('modul','I.T.A.2');
                ...
                if (!$this->image_lib->resize()) {
                    ...
                    $file = $res['full_path'];
                    $file_ext = $res['file_ext'];
                    $final_file_name = trim(trim($nik)) .'kpen_k_'.$kdkartu. $file_ext;
                    if (file_exists($file_path . $final_file_name)) {
                        unlink($file_path . $final_file_name);
                    }
                    rename($file, $file_path . $final_file_name);
                ...
                $this->db->insert('sc_his.kartu_pendukung',$insert_this);
    ...
    function save_kartupendukung_update(){
        ...
        if (!empty($_FILES['kartupendukung_upd_attachment']['name'])) {
            $config['upload_path'] = $path;
            $config['allowed_types'] = 'gif|jpg|png|jpeg';
            $config['max_size'] = '5000';
            $config['max_width'] = '3624';
            $config['max_height'] = '3200';
            $this->upload->initialize($config);
            if (!$this->upload->do_upload('kartupendukung_upd_attachment')) {
                $getResult = array('status' => false, 'messages' => 'Gambar Tidak Sesuai</br>
					Format yang sesuai: * Ukuran File yang di ijinkan max 2MB</br>');
                json_encode($getResult);
                //INSERT TRX ERROR
                ...
                $this->db->where('id',$id);
                $this->db->update('sc_his.kartu_pendukung',$insert_this);
    ...
    function save_kartupendukung_delete(){
        ...
        $dtl = $this->m_candidateos->q_kartupendukung(" and id='$id'")->getRowArray();
        $final_file_name = trim($dtl['attachment']);
        $path = "./assets/img/kartupendukung/";
        if (!is_dir($path)) {
            mkdir($path, 0777, TRUE);
        ...
        $this->db->where('id',$id);
        $this->db->delete('sc_his.kartu_pendukung');
    ...
    {
        $data['title']='Halaman Persetujuan Karyawan Baru (Input By Outsource)';

        /* CODE UNTUK VERSI*/
        $nama=trim($this->session->get('nama'));
        $kodemenu='I.T.A.3'; $versirelease='I.T.A.3/RELEASE.401'; $releasedate=date('2019-04-12 00:00:00');
        $versidb=$this->fiky_version->version($kodemenu,$versirelease,$releasedate,$nama);
        $x=$this->fiky_menu->menus($kodemenu,$versirelease,$releasedate);
        $data['x'] = $x['rows']; $data['y'] = $x['res']; $data['t'] = $x['xn'];
        $data['kodemenu']=$kodemenu; $data['version']=$versidb;
        ...
        $roleid = $this->session->get('roleid');
        $data['akses']=$this->m_global->list_akses_role($roleid)->getRowArray();
        return $this->template->render('trans/candidateos/v_approvalpage_karyawan',$data);
    }

    public function approvalpage_list()
    {
        //AHC ADMIN HC // ML SUPER ADMIN IRGA
        $nama = trim($this->session->get('nama'));
        $role = trim($this->session->get('roleid'));
...
<div class="dropdown">
    ...
    <ul class="dropdown-menu" role="menu" aria-labelledby="menu1" style="background: #b3efff">
        <li role="presentation"><a role="menuitem" title="Detail" href="#" onclick="detailNik('."'".trim($person->nik)."'".');"><i class="fa fa-bars"></i> Detail </a></li>
        <li role="presentation"><a role="menuitem" href="'.base_url('trans/candidateos/hc_approved').'/'.trim($person->nik).'" title="Setujui Data" onclick="return confirm('."'Data Karyawan Ini akan diproses,jika akan terbentuk nik?  ".trim($person->nik)."'".')"><i class="fa fa-check"></i> Approval </a></li>
        <li role="presentation"><a role="menuitem" href="'.base_url('trans/candidateos/ajax_delete').'/'.trim($person->nik).'"  onclick="return confirm('."'Apakah Anda Yakin Cancel Ini?  ".trim($person->nik)."'".')"><i class="fa fa-close"></i> Cancel </a></li>
        <li role="presentation"><a role="menuitem" href="'.base_url('trans/candidateos/hc_sendback').'/'.trim($person->nik).'"  onclick="return confirm('."'Apakah Anda Mengembalikan Dokumen Ini Untuk Di Proses Ulang  ".trim($person->nik)."'".')"><i class="fa fa-arrow-left"></i> Send Back </a></li>
    </ul>
</div>
               ';
            } else {
...
<div class="dropdown">
    ...
    function cekavaiablenik(){
        ...
        if ($data->key=='1203jD0j120dkjjKODNOoimdi)D(J)Jmjid0sjd0ijme09wjei0kjisdjfDSojiodksOjO') {
            ...
            $inputby = $nama;
            $inputdate = date('Y-m-d H:i:s');
            $cek = $this->m_candidateos->q_ceknik(" and nik='$nik'")->getNumRows();
            if ($cek>0) {
                $getResult = array('status' => false, 'messages' => 'Data Nik Sudah Ada, Anda tidak diperkenankan memakai nik tsb!!');
                echo json_encode($getResult);
            } else {
                $getResult = array('status' => true, 'messages' => 'Data Nik Tersedia');
                echo json_encode($getResult);
            }
        } else {
