...
class Ess_karyawan extends BaseController
{
    public function index()
    {
        ...
        $kodemenu='I.E.A.1'; $versirelease='I.E.A.1/RELEASE.401'; $releasedate=date('2020-04-12 00:00:00');
        ...
        $data['akses']=$this->m_role->list_akses_permission($roleid)->getRowArray();
        return $this->template->render('trans/ess_karyawan/v_karyawan',$data);
    }

    public function ajax_list()
    {
        $list = $this->m_ess_karyawan->get_datatables();
        $data = array();
        $no = $_POST['start'];
        $nama=trim($this->session->get('nama'));
        foreach ($list as $person) {
            ...
            <div class="dropdown">
                ...
                <ul class="dropdown-menu" role="menu" aria-labelledby="menu1" style="background: #b3efff">
                    ...
                    <li role="presentation"><a role="menuitem" title="Upload Data Pendukung" onclick="detailAtributNik('."'".trim($person->nik)."'".');"><i class="fa fa-download"></i> Upload Data Pendukung </a></li>
                </ul>
            </div>
                           ';
            $row[] = $person->nik;
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->m_ess_karyawan->count_all(),
            "recordsFiltered" => $this->m_ess_karyawan->count_filtered(),
            "data" => $data,
        );
    ...
    public function input(){
        /* CODE UNTUK VERSI*/
        $nama=trim($this->session->get('nama'));
        $kodemenu='I.E.A.1'; $versirelease='I.E.A.1/RELEASE.401'; $releasedate=date('2019-04-12 00:00:00');
        $versidb=$this->fiky_version->version($kodemenu,$versirelease,$releasedate,$nama);
        $x=$this->fiky_menu->menus($kodemenu,$versirelease,$releasedate);
        $data['x'] = $x['rows']; $data['y'] = $x['res']; $data['t'] = $x['xn'];
        $data['kodemenu']=$kodemenu; $data['version']=$versidb;
        ...
        $data['type']="INPUT";
        $data['nama']=$nama;
        return $this->template->render('trans/ess_karyawan/v_input_karyawan',$data);
    }
    ...
    function showDetailNik () {
        ...
        $param = " and nik='$docno'";
        $dtl = $this->m_ess_karyawan->list_karyawan_param($param);
    ...
    function saveDataMasterKaryawan(){
        ...
        if (!empty($_FILES['cimage']['name'])) {
            ...
            if(!$this->upload->do_upload('cimage')){
                $result = array('status' => false, 'messages' => 'Gambar Tidak Sesuai</br>
                ...
                if ( ! $this->image_lib->resize()){
                    $result = array('status' => false, 'messages' => $this->image_lib->display_errors('', ''));
                    echo json_encode($result);
                } else {
            ...
            if ($type==='INPUT'){
                ...
                if ($this->db->insert('sc_mst.t_karyawan', $info)) {
                    $paramerror=" and userid='$nama' and modul='I.T.A.1'";
                    $dtlerror=$this->m_trxerror->q_trxerror($paramerror)->getRowArray();
                    $result = array('status' => true, 'messages' => 'Sukses Di Proses'.' Nomor: '.trim($dtlerror['nomorakhir1']));
                    echo json_encode($result);
                } else {
                    $result = array('status' => false, 'messages' => 'Data Gagal Di Proses Ada Kesalahan Data 2');
                    echo json_encode($result);
                }
            } else if ($type==='EDIT' or $type==='EDIT_RESIGN'){
                $infoX = array(
                    'branch' => 'JTS',
                    'nmlengkap' => $nmlengkap,
                    'callname' => $callname,
                ...
                $this->db->where('nik','DISABLE EDIT');
                if ($this->db->update('sc_mst.t_karyawan', $infoX)) {
                    $paramerror=" and userid='$nama' and modul='I.T.A.1'";
                    $dtlerror=$this->m_trxerror->q_trxerror($paramerror)->getRowArray();
                    $result = array('status' => true, 'messages' => 'Sukses Di Proses'.' Nomor: '.trim($dtlerror['nomorakhir1']));
                    echo json_encode($result);
            ...
            if ($type==='INPUT'){
                ...
                $this->db->where('nik','DISABLE_EDIT');
                if ($this->db->update('sc_mst.t_karyawan', $infoX)) {
                    $paramerror=" and userid='$nama' and modul='I.T.A.1'";
                    $dtlerror=$this->m_trxerror->q_trxerror($paramerror)->getRowArray();
                    $result = array('status' => true, 'messages' => 'Sukses Di Proses'.' Nomor: '.trim($dtlerror['nomorakhir1']));
    ...
    {
        $data = $this->m_ess_karyawan->get_by_id($id)->row();
    ...
    public function detail($id)
    {
        ...
        $data['list_opt_atasan']=$this->m_ess_karyawan->list_karyawan()->getResult();
        ...
        $data['list_finger']=$this->m_ess_karyawan->q_finger()->getResult();//
        $data['list_kanwil']=$this->m_ess_karyawan->q_kanwil()->getResult();//
        ...
        $data['title'] = "Detail Karyawan";
        //$data['dtl'] = $this->m_ess_karyawan->get_dtl_id($id)->getRowArray();
        $data['lp'] = $this->m_ess_karyawan->get_dtl_id($id)->getRowArray();
        return $this->template->render('trans/ess_karyawan/v_detailhrdkary',$data);
    }
    ...
    public function detail_resign($id)
    {
        ...
        $data['list_finger']=$this->m_ess_karyawan->q_finger()->getResult();//
        $data['list_kanwil']=$this->m_ess_karyawan->q_kanwil()->getResult();//
        /// $data['list_wilnom']=$this->m_ess_karyawan->q_wilayah_nominal($p=null)->getResult();// wilayah nominal
        ...
        //$data['dtl'] = $this->m_ess_karyawan->get_dtl_id($id)->getRowArray();
        $data['lp'] = $this->m_ess_karyawan->get_dtl_id($id)->getRowArray();
        return $this->template->render('trans/ess_karyawan/v_detailhrdkary_resign',$data);
    }
    ...
    public function edit($id){
        ...
        $data['list_kanwil']=$this->m_ess_karyawan->q_kanwil()->getResult();//
        $data['title'] = "Edit Karyawan";
        $data['dtl'] = $this->m_ess_karyawan->get_by_id($id)->getRowArray();
        return $this->template->render('trans/ess_karyawan/v_editkary',$data);
    }
    ...
    public function ajax_add()
    {
        ...
        $besaranptkp1=$this->m_ess_karyawan->q_besaranptkp($status_ptkp)->getRowArray();
        ...
        $cek=$this->m_ess_karyawan->cek_exist($nik);
        if ($cek>0) {
            redirect('trans/ess_karyawan/index/exist');
        } else {
            $insert = $this->m_ess_karyawan->save($data);
            echo json_encode(array("status" => TRUE));
            redirect('trans/ess_karyawan/index/success');
        }
    ...
    public function ajax_update()
    {
        ...
        $this->m_ess_karyawan->update(array('nik' => $this->request->getPost('nik')), $data);
        echo json_encode(array("status" => TRUE));
        redirect('trans/ess_karyawan/index/upsuccess');
    ...
    {
        //$this->m_ess_karyawan->delete_by_id($id);
        //echo json_encode(array("status" => TRUE));
        $info = array('resign' => 'YES');
        $this->db->where('nik',$id);
        $this->db->update('sc_mst.t_karyawan',$info);
        redirect('trans/ess_karyawan/index/del');
    }

    public function excel_listkaryawan(){
        $datane = $this->m_ess_karyawan->list_karyawan_param2("");
        ...
        $this->fiky_excel->set_header(array(
            ...
            'PROVINSI KTP','KOTA/KAB KTP','KECAMATAN KTP','KEL/DESA KTP','RT/RW KTP','KODEPOS KTP','ALAMAT KTP','ALAMAT LENGKAP KTP',
            'TYPE DOMISILI','TRANSPORTASI',
            'PROVINSI DOMISILI','KOTA/KAB DOMISILI','KECAMATAN DOMISILI','KEL/DESA DOMISILI','RT/RW DOMISILI','KODEPOS DOMISILI','ALAMAT DOMISILI','ALAMAT LENGKAP DOMISILI',
            ...
            'ID BPJSKES','CABANG BPJSKES','ID BPJSNAKER','CAB BPJS NAKER','ATASAN1','ATASAN2','ATASAN3','VAKSIN','SISA CUTI',
            'PENYAKIT BAWAAN','KELUARGA BERPENYAKIT BAWAAN','NAKES','RESIGN','ALASAN RESIGN','TGL KELUAR'
        ));
        ...
        $this->fiky_excel->set_column(array(
            ...
            'namaprovktp','namakotakabktp','namakecktp','namakeldesaktp','rtrwktp','id_posktp','desc_ktp','alamatlengkapktp',
            'typedomisili','transportasi',
            'namaprovdom','namakotakabdom','namakecdom','namakeldesadom','rtrwdom','id_posdom','desc_domisili','alamatlengkapdomisili',
            ...
            'id_bpjs_kes','cab_bpjs_kes','id_bpjs_naker','cab_bpjs_naker','nmatasan1','nmatasan2','nmatasan3','area_covid','sisacuti',
            'penyakitbawaan','nmkeluargapenyakitbawaan','nakes','resign','alasan_resign','tglkeluarkerja1'
        ));
    ...
    function up_foto(){
        ...
        $nm=$this->m_ess_karyawan->get_by_id($nik)->getRowArray();
        $nama=$nm['nmlengkap'];
        ...
        if(!$this->upload->do_upload('gambar')){
            ...
            if ( ! $this->image_lib->resize()){
                ...
                $info=array('image'=>$gambar);
                //update foto pegawai
                $this->m_ess_karyawan->save_foto($nik,$info);
            ...
            redirect("trans/ess_karyawan/detail/$nik");
        }
        //tampilkan pesan
    ...
    public function excel_karyborong(){
        ...
        $datane=$this->m_ess_karyawan->list_karyborong();
    ...
    function simpanmutasi (){
        ...
        redirect("trans/ess_karyawan/detail/$nik/success");
    ...
    function add_bpjs(){
        ...
        if ($cek>0){
            redirect("trans/ess_karyawan/detail/$nik/kode_failed");;
        } else {
    ...
    function hps_bpjs($nik,$id_bpjs){
        ...
        redirect("trans/ess_karyawan/detail/$nik/del_success");
    }
    ...
    function karyawan_self(){
        ...
        $data['list_opt_atasan']=$this->m_ess_karyawan->list_karyawan()->getResult();
        $data['list_opt_ptkp']=$this->m_bpjs->list_ptkp()->getResult();
        //$data['list_opt_ptkp']=$this->m_ess_karyawan->list_ptkp()->getResult();
        $data['list_opt_grp_gaji']=$this->m_group_penggajian->q_group_penggajian()->getResult();
        $data['list_opt_bank']=$this->m_bank->q_bank()->getResult();
        $data['list_resignkary']=$this->m_ess_karyawan->list_karyresgn()->getResult();//
        $data['list_borong']=$this->m_ess_karyawan->list_karyborong()->getResult();//
        $data['calon_karyawan']=$this->m_calonkaryawan->q_maxktpcalon()->getResult();//
        $data['list_finger']=$this->m_ess_karyawan->q_finger()->getResult();//
        $data['list_kanwil']=$this->m_ess_karyawan->q_kanwil()->getResult();//
        $data['list_self']=$this->m_ess_karyawan->get_dtl_id($id)->getResult();
        ...
        return $this->template->render('trans/ess_karyawan/v_karyawan_self',$data);
    }
    ...
    function detail_self($id)
    {
        ...
        //$data['dtl'] = $this->m_ess_karyawan->get_dtl_id($id)->getRowArray();
        $data['lp'] = $this->m_ess_karyawan->get_dtl_id($id)->getRowArray();
        return $this->template->render('trans/ess_karyawan/v_detailhrdkary_self',$data);
    }
    function edit_self($id)
    {
        ...
        $data['title'] = "Edit Karyawan";
        $data['dtl'] = $this->m_ess_karyawan->get_by_id($id)->getRowArray();
        return $this->template->render('trans/ess_karyawan/v_editkary_self',$data);
    }
    ...
    function update_self_person(){
        ...
        redirect("trans/ess_karyawan/karyawan_self/upsuccess");
    }
    ...
    function add_riwayat_keluarga(){
        ...
        $inputby=$this->session->get('nama');
        if (empty($kdkeluarga)){
            redirect("trans/ess_karyawan/index/");
        }
        ...
        redirect("trans/ess_karyawan/detail/$nik/rep_succes");
    ...
    function hps_riwayat_kesehatan($nik,$no_urut){
        ...
        redirect("trans/ess_karyawan/detail/$nik/del_succes");
    }
    ...
    {
        $list = $this->m_ess_karyawan->get_t_resign_view();
        $data = array();
        $no = $_POST['start'];
        $nama=trim($this->session->get('nama'));
        foreach ($list as $person) {
    ...
    public function detail_atribut_karyawan(){
        ...
        $paramerror=" and userid='$nama' and modul='I.E.A.1'";
        $dtlerror=$this->m_trxerror->q_trxerror($paramerror)->getRowArray();
        ...
        $data['docno'] =$docno;
        /* RIWAYAT PENDIDIKAN */
        $data['riwayat_pendidikan'] = $this->m_ess_karyawan->q_riwayat_pendidikan(" and nik='$docno'")->getResult();
        $data['m_pendidikan'] = $this->m_ess_karyawan->q_m_pendidikan(" ")->getResult();
        /* KARTU PENDUKUNG */
        $data['list_kartupendukung'] = $this->m_ess_karyawan->q_kartupendukung(" and nik='$docno'")->getResult();
        $data['m_kartupendukung'] = $this->m_ess_karyawan->q_m_kartupendukung(" ")->getResult();

        $data['title_abscut'] = 'Laporan Absensi Cuti/Sakit/Dinas';
        $data['title_abstelat'] = 'Laporan Keterlambatan & Pulang Awal Individu';
        $data['title_covidtrack'] = 'Laporan Historis Kesehatan Karyawan';
        $data['list_abscut'] = $this->m_abscut->q_abscut(" and nik='$docno' and trim(idtypecuti) not in ('PA','DT')")->getResult();
        $data['list_abstel'] = $this->m_abscut->q_abscut(" and nik='$docno' and trim(idtypecuti) in ('PA','DT')")->getResult();
        $data['list_covidtrack'] = $this->m_ess_karyawan->q_covidtrack(" and nik='$docno' and trim(idtypecuti) in ('VK1','VK2','IM','CP','SC')")->getResult();


        $paramerror=" and userid='$nama'";
        $dtlerror=$this->m_trxerror->q_deltrxerror($paramerror);
        return $this->template->render('trans/ess_karyawan/v_detail_atribut_karyawan',$data);
    ...
    function save_pendidikanf(){
        ...
        if (!empty($file->getName())) {
            ...
            if(!$validateImg){
                ...
                return redirect()->to(base_url("trans/ess_karyawan/detail_atribut_karyawan/?docno=$nik"));
