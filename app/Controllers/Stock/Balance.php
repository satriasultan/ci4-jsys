...
class Balance extends BaseController
{
    ...
    function api_sti_allocation_map(){
        ...
        $param=" and (trim(idbarang)||trim(batch) = '$varGet' $paramglobal1 $parameterx ) order by idbarang asc";
        //$param_row=" and (idbarang  in (select trim(idbarang) from sc_mst.stkgdw where idlocation='$loccode' and trim(idbarang)||trim(batch)='$varGet' group by idbarang)) order by idbarang asc";
        $param_row=" and trim(idbarang)||trim(batch)  ='$varGet' limit 1";

        $databranch = $this->m_global->q_master_branch();
        //$datamst = $this->m_balance->q_item_param($param_row);
        $datamst = $this->m_balance->q_stkgdw($param_row);
...
//                }
    ...
    function input_stockopname(){
        ...
        if (!$this->request->getPost('dateOpname')) {
            return redirect()->to(base_url('/stock/balance/stockopname'));
        }

        $data['dateOpname'] = date('d-m-Y',strtotime($this->request->getPost('dateOpname')));
        //auto insert unit
        $pterror = " and userid='$nama'";
        $this->m_trxerror->q_deltrxerror($pterror);
    ...
    function showing_detail_so(){
        $id = $this->request->getGet('id');
        $dateOpname = date('Y-m-d',strtotime($this->request->getGet('dateOpname')));

        $param = " and trim(idlocation)||trim(idarea)||trim(batch)||trim(idbarang) 
in (select trim(idlocation)||trim(idarea)||trim(batch)||trim(idbarang) from sc_mst.stkgdw where id=$id)";
        $paramdate = " and to_char(trxdate,'yyyy-mm-dd')<='$dateOpname'";
        $data = $this->m_balance->q_balance_so($param,$paramdate)->getRow();
        echo (json_encode($data,JSON_PRETTY_PRINT));
    }
    ...
    function saveStockOpname(){
        ...
        $description = strtoupper(trim($this->request->getPost('description')));
        $batchnew = strtoupper(trim($this->request->getPost('batchnew')));
        $inputby = $nama;
        $inputdate = date('Y-m-d H:i:s');
        $type = trim($this->request->getPost('type'));
        $dtl = $this->m_balance->q_item_param(" and idbarang='$idbarangmove'")->getRowArray();
        $builder = $this->db->table('sc_trx.item_stock_opname');
        // CHECK INPUT TYPE
        if ($type==='UPDATE') {
                $info = array(
