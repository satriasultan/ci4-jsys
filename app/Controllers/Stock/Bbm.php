...
class Bbm extends BaseController
{
    ...
    function update_stock_bbm()
    {
        ...
        $param_blockgudang = " and trim(idlocation)!='$loccode' ";
        $dtlblock = $this->m_bbm->q_trx_item_bbm_mst($param_blockgudang)->getNumRows();

        $builder_trxerror = $this->db->table('sc_mst.trxerror');
        $builder = $this->db->table('sc_trx.item_bbm_mst');
        if ($dtl>0 or $dtlblock>0){
...
//        $this->fiky_qrcode->proses($logo,$QR,$tempdir.$docno.'.png');
        ...
        //file_get_contents("http://10.0.151.37/ci4-inventory/api/validatorpush/renderpdf"."/".$docno);
        //$data['urlx'] = 'http://10.0.151.37/ci4-inventory/assets/files/lpb_print/'.$docno.'.pdf';
...
from sc_trx.item_bbm_dtl where docno='$docno');
    ...
    function loadItemVoidLpb(){
        ...
        if ($data->key=='1203jD0j120dkjjKODNOoimdi)D(J)Jmjid0sjd0ijme09wjei0kjisdjfDSojiodksOjO') {
            ...
            $count_block_gudang = $this->m_bbm->q_trx_item_bbm_mst(" and docno='$docno' and idlocation='$loccode'")->getNumRows();

            if ($count > 0 and $countvoid<=0 and $cek_bbk<=0 and $count_block_gudang > 0) {
                // CHECK INPUT TYPE
                $this->db->query("
         INSERT INTO sc_tmp.item_void_mst(
docno, docref, doctype, idlocation, docdate, description, status, inputby, inputdate, cancelby, canceldate, docnotmp, holdrow, holdby, updatedate, updateby, idsection, descsection, docjns) 
    ...
    function list_tmp_void_dtl(){
        ...
        foreach ($list as $lm) {
            ...
            $row[] = ' <div class="form-check">
                            <input type="checkbox" class="form-check-input" id="checkvoid[]" name="checkvoid[]" value="'.trim($lm->id).'">
                        </div>';
            /*
            $row[] = '
            <div class="dropdown">
...
delete from sc_tmp.item_void_dtl where docno='$nama';");
    ...
    function progresVoid(){
        ...
        $trx_build->insert($infotrxerror);
        $getResult = array('status' => true, 'messages' => 'Calculation Success By' . $idnya);
        echo json_encode($getResult);
    }
...
from sc_trx.item_po_dtl where docno = '$pono' and status in ('A','S') and id not in (select idpodtl from sc_tmp.item_bbm_dtl where docno='$nama'));");
    ...
    function import_lbm(){
        $data['title']="Import Data LPB Dari Winacc ";
        //echo ;
        /* CODE UNTUK VERSI*/
        $nama=trim($this->session->get('nama'));
        $loccode=trim($this->session->get('loccode'));
            ...
            if (!$validateFile){
                ...
                for ($row = 2; $row <= $highestRow; $row++){  				//  Read a row of data into an array
                    ...
                    $data = array(
                        ...
                        'doctype' => 'IN',
                        'docjns' => 'LPB',
                        'trxdate' => date('Y-m-d',strtotime(str_replace('/','-',trim($rowData[0][10])))),
                        ...
                        'senddate' =>  date('Y-m-d',strtotime(str_replace('/','-',trim($rowData[0][11])))),
                        'uniqueidobd' => trim($rowData[0][5]),
                        'holdrow' => 'NO',
                        'inputby'=>$nama,
                        'inputdate'=>date("Y-m-d H:i:s"),
                        'status' => 'I',
                        'batch' => trim($rowData[0][12]),
                    );
