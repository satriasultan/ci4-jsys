...
var table;
//"use strict";
function tableMkelompokbarang(){
    // var lg = languageDatatable;
    var initTable = function () {
        var table = $('#tmkelompokbarang');
        table.DataTable({
            ...
            "ajax": {
                "url": HOST_URL + 'master/data/list_kelompokbarang',
                "type": "POST",
                "data": function(data) {
...
function add_kelompokbarang()
{
    ...
    $('.modal-title').text('Input Master Kelompok Barang'); // Set Title to Bootstrap modal title
    $('[name="type"]').val('INPUT');
    $('[name="id"]').val();
    $('[name="idkelompokbarang"]').prop("required", true);
    $('#btnSave').removeClass("btn-danger").addClass("btn-primary").text('Simpan');
}
function update_kelompokbarang(id)
{
    save_method = 'update';
    ...
    $.ajax({
        url: HOST_URL + 'master/data/showing_data_kelompokbarang' + '/' + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            ...
            $('[name="id"]').val(data.id);
            $('[name="idkelompokbarang"]').val(data.idkelompokbarang).prop("readonly", true);
            $('[name="nmkelompokbarang"]').val(data.nmkelompokbarang.trim()).prop("readonly", false);
            ...
            $('.modal-title').text('Update Master Kelompok Barang'); // Set title to Bootstrap modal title
...
function delete_kelompokbarang(id)
{
    save_method = 'delete';
    //$('#modal_form').removeData('bs.modal');
    var validator = $('#form').data('bootstrapValidator');
    ...
    $.ajax({
        ...
        success: function(data)
        {
            ...
            $('[name="id"]').val(data.id);
            $('[name="idkelompokbarang"]').val(data.idkelompokbarang).prop("readonly", true);
            $('[name="nmkelompokbarang"]').val(data.nmkelompokbarang).prop("readonly", true);
            $('[name="chold"]').val(data.chold.trim());
            ...
            //$('[name="dob"]').datepicker('update',data.dob);
            $('#modal_form').modal('show'); // show bootstrap modal when complete loaded
            $('.modal-title').text('Delete kelompokbarang'); // Set title to Bootstrap modal title
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            if (validator.isValid()) {
                ...
                var validkelompokbarang= $('[name="idkelompokbarang"]').val();
                var valnmkelompokbarang = $('[name="nmkelompokbarang"]').val();
                ...
                var fillData = {
                    ...
                    'body': {
                        id: valID,
                        idkelompokbarang: validkelompokbarang,
                        nmkelompokbarang: valnmkelompokbarang,
                        // pbiaya: valpbiaya,
                        chold: valchold,
                        type: valtype
                    },
                ...
                $.ajax({
                    type: "POST",
                    url: HOST_URL + 'master/data/saveKelompokBarang' + '',
                    dataType: 'json',
                    ...
                    success: function (datax) {
                        if (datax.status) {
                            Swal.fire({
                                ...
                                /* Read more about isConfirmed, isDenied below */
                                if (result.isConfirmed) {
                                    // window.kelompokbarang.replace(HOST_URL + 'trans/referensisdm')
                                    reload_table();
...
// ID POSTION
var defaultInitialGroup = $('[name="idkelompokbarang"]').val();
