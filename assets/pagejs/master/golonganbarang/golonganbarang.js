...
var table;
//"use strict";
function tableMgolonganbarang(){
    // var lg = languageDatatable;
    var initTable = function () {
        var table = $('#tmgolonganbarang');
        table.DataTable({
            ...
            "ajax": {
                "url": HOST_URL + 'master/data/list_golonganbarang',
                "type": "POST",
                "data": function(data) {
...
function add_golonganbarang()
{
    ...
    $('.modal-title').text('Input Master Golongan Barang'); // Set Title to Bootstrap modal title
    $('[name="type"]').val('INPUT');
    $('[name="id"]').val();
    $('[name="idgolonganbarang"]').prop("required", true);
    $('#btnSave').removeClass("btn-danger").addClass("btn-primary").text('Simpan');
}
function update_golonganbarang(id)
{
    save_method = 'update';
    ...
    $.ajax({
        url: HOST_URL + 'master/data/showing_data_golonganbarang' + '/' + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            ...
            $('[name="id"]').val(data.id);
            $('[name="idgolonganbarang"]').val(data.idgolonganbarang).prop("readonly", true);
            $('[name="nmgolonganbarang"]').val(data.nmgolonganbarang.trim()).prop("readonly", false);
            ...
            $('.modal-title').text('Update Master Golongan Barang'); // Set title to Bootstrap modal title
...
function delete_golonganbarang(id)
{
    save_method = 'delete';
    //$('#modal_form').removeData('bs.modal');
    var validator = $('#form').data('bootstrapValidator');
    ...
    $.ajax({
        ...
        success: function(data)
        {
            ...
            $('[name="id"]').val(data.id);
            $('[name="idgolonganbarang"]').val(data.idgolonganbarang).prop("readonly", true);
            $('[name="nmgolonganbarang"]').val(data.nmgolonganbarang).prop("readonly", true);
            $('[name="chold"]').val(data.chold.trim());
            ...
            //$('[name="dob"]').datepicker('update',data.dob);
            $('#modal_form').modal('show'); // show bootstrap modal when complete loaded
            $('.modal-title').text('Delete golonganbarang'); // Set title to Bootstrap modal title
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            if (validator.isValid()) {
                ...
                var validgolonganbarang= $('[name="idgolonganbarang"]').val();
                var valnmgolonganbarang = $('[name="nmgolonganbarang"]').val();
                ...
                var fillData = {
                    ...
                    'body': {
                        id: valID,
                        idgolonganbarang: validgolonganbarang,
                        nmgolonganbarang: valnmgolonganbarang,
                        // pbiaya: valpbiaya,
                        chold: valchold,
                        type: valtype
                    },
                ...
                $.ajax({
                    type: "POST",
                    url: HOST_URL + 'master/data/saveGolonganBarang' + '',
                    dataType: 'json',
                    ...
                    success: function (datax) {
                        if (datax.status) {
                            Swal.fire({
                                ...
                                /* Read more about isConfirmed, isDenied below */
                                if (result.isConfirmed) {
                                    // window.golonganbarang.replace(HOST_URL + 'trans/referensisdm')
                                    reload_table();
...
// ID POSTION
var defaultInitialGroup = $('[name="idgolonganbarang"]').val();
