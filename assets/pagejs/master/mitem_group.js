...
var table;
//"use strict";
function tableMitemGroup(){
    // var lg = languageDatatable;
    var initTable = function () {
        var table = $('#tmitem_group');
        table.DataTable({
            ...
            "ajax": {
                "url": HOST_URL + 'master/item/list_mgroup',
                "type": "POST",
                "data": function(data) {
...
function add_mgroup()
{
    ...
    $('.modal-title').text('Input New Item Group'); // Set Title to Bootstrap modal title
    $('[name="type"]').val('INPUT');
    $('[name="id"]').val();
    $('[name="idgroup"]').prop("required", true);
    $('[name="nmgroup"]').prop("required", true);
    $('[name="grouptype"]').prop("readonly", true);
...
function update_mgroup(id)
{
    ...
    $.ajax({
        url: HOST_URL + 'master/item/showing_data_mgroup' + '/' + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            ...
            $('[name="id"]').val(data.id);
            $('[name="idgroup"]').val(data.idgroup).prop("readonly", true);
            $('[name="nmgroup"]').val(data.nmgroup.trim()).prop("readonly", false);
...
function delete_mgroup(id)
{
    ...
    $.ajax({
        ...
        success: function(data)
        {
            ...
            $('[name="id"]').val(data.id);
            $('[name="idgroup"]').val(data.idgroup).prop("readonly", true);
            $('[name="nmgroup"]').val(data.nmgroup.trim()).prop("readonly", true);
            $('[name="grouptype"]').val(data.nmgroup.trim()).prop("readonly", true);
            $('[name="chold"]').val(data.chold.trim());
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            if (validator.isValid()) {
                ...
                var valnmgroup= $('[name="nmgroup"]').val();
                var valgrouptype= $('[name="grouptype"]').val();
                ...
                var fillData = {
                    ...
                    'body': {
                        ...
                        grouptype: valgrouptype,
                        chold: valchold,
                    },
                };
                $.ajax({
                    type: "POST",
                    url: HOST_URL + 'master/item/saveMgroup' + '',
                    dataType: 'json',
