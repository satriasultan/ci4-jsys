...
var table;
//"use strict";
function tableMprincipal(){
    // var lg = languageDatatable;
    var initTable = function () {
        var table = $('#tmprincipal');
        table.DataTable({
            ...
            "ajax": {
                "url": HOST_URL + 'master/data/list_principal',
                "type": "POST",
                "data": function(data) {
...
function add_principal()
{
    ...
    $('.modal-title').text('Input Master Principal'); // Set Title to Bootstrap modal title
    $('[name="type"]').val('INPUT');
    $('[name="kdsupplier"]').val(null).trigger('change').prop('disabled',false);
    $('[name="id"]').val();
    $('[name="idprincipal"]').prop("required", true);
    $('#btnSave').removeClass("btn-danger").addClass("btn-primary").text('Simpan');
}
function update_principal(id)
{
    save_method = 'update';
    ...
    $.ajax({
        url: HOST_URL + 'master/data/showing_data_principal' + '/' + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            ...
            $('[name="id"]').val(data.id);
            $('[name="idprincipal"]').val(data.idprincipal).prop("readonly", true);
            $('[name="nmprincipal"]').val(data.nmprincipal.trim()).prop("readonly", false);
            $('[name="chold"]').val(data.chold.trim());
            
            $('[name="alamat"]').val(data.alamat).prop("readonly", true);
            $('[name="phone"]').val(data.phone).prop("readonly", true);
            $.ajax({
                type: 'GET',
                url: HOST_URL + 'api/globalmodule/list_supplier_new' + '?var=' + data.kdsupplier,
                dataType: 'json',
                delay: 250,
            }).then(function (datax) {
            ...
            // Tambahkan data alamat dan phone ke object
                ...
                supplierData.alamat = data.alamat;
            ...
            $('.modal-title').text('Update Master Principal'); // Set title to Bootstrap modal title
...
function delete_principal(id)
{
    ...
    $.ajax({
        ...
        success: function(data)
        {
            ...
            $('[name="id"]').val(data.id);
            $('[name="idprincipal"]').val(data.idprincipal).prop("readonly", true);
            $('[name="nmprincipal"]').val(data.nmprincipal).prop("readonly", true);
            $('[name="chold"]').val(data.chold.trim());
            $.ajax({
                type: 'GET',
                url: HOST_URL + 'api/globalmodule/list_supplier_new' + '?var=' + data.kdsupplier,
                ...
                var option = new Option(datax.items[0].nmsupplier, datax.items[0].kdsupplier, true, true);
                $('[name="kdsupplier"]').append(option).trigger('change').prop("disabled", true);

                // manually trigger the `select2:select` event
                $('[name="kdsupplier"]').trigger({
                    type: 'select2:select',
                    params: {
                        data: datax
                    }
            ...
            $('[name="alamat"]').val(data.alamat).prop("disabled", true);
            $('[name="phone"]').val(data.phone).prop("disabled", true);
            $('.chold').prop("disabled", true);
            $('#btnSave').removeClass("btn-primary").addClass("btn-danger").text('Delete');
            //$('[name="dob"]').datepicker('update',data.dob);
            $('#modal_form').modal('show'); // show bootstrap modal when complete loaded
            $('.modal-title').text('Delete principal'); // Set title to Bootstrap modal title
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            if (validator.isValid()) {
                ...
                var validprincipal= $('[name="idprincipal"]').val();
                var valnmprincipal = $('[name="nmprincipal"]').val();
                var valkdsupplier = $('[name="kdsupplier"]').val();
                var alamat = $('[name="alamat"]').val();
                var valphone = $('[name="phone"]').val();
                ...
                var fillData = {
                    ...
                    'body': {
                        id: valID,
                        idprincipal: validprincipal,
                        nmprincipal: valnmprincipal,
                        kdsupplier: valkdsupplier,
                        alamat: alamat,
                        phone: valphone,
                        // pbiaya: valpbiaya,
                        chold: valchold,
                ...
                $.ajax({
                    type: "POST",
                    url: HOST_URL + 'master/data/savePrincipal' + '',
                    dataType: 'json',
                    ...
                    success: function (datax) {
                        if (datax.status) {
                            Swal.fire({
                                ...
                                /* Read more about isConfirmed, isDenied below */
                                if (result.isConfirmed) {
                                    // window.principal.replace(HOST_URL + 'trans/referensisdm')
                                    reload_table();
...
// ID POSTION
var defaultInitialGroup = $('[name="idprincipal"]').val();
...
$("#kdsupplier").select2({
    ...
    if (e.params && e.params.data) {
        ...
        $("#alamat").val(selectedData.alamat || '').prop('disabled', true);
