...
var table;
//"use strict";
function tableMjenisproduk(){
    // var lg = languageDatatable;
    var initTable = function () {
        var table = $('#tmjenisproduk');
        table.DataTable({
            ...
            "ajax": {
                "url": HOST_URL + 'master/data/list_jenisproduk',
                "type": "POST",
                "data": function(data) {
...
function add_jenisproduk()
{
    ...
    $('.modal-title').text('Input Master Jenis Produk'); // Set Title to Bootstrap modal title
    $('[name="type"]').val('INPUT');
    $('[name="id"]').val();
    $('[name="idjenisproduk"]').prop("required", true);
    $('#btnSave').removeClass("btn-danger").addClass("btn-primary").text('Simpan');
}
function update_jenisproduk(id)
{
    save_method = 'update';
    ...
    $.ajax({
        url: HOST_URL + 'master/data/showing_data_jenisproduk' + '/' + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            ...
            $('[name="id"]').val(data.id);
            $('[name="idjenisproduk"]').val(data.idjenisproduk).prop("readonly", true);
            $('[name="nmjenisproduk"]').val(data.nmjenisproduk.trim()).prop("readonly", false);
            ...
            $('.modal-title').text('Update Master Jenis Produk'); // Set title to Bootstrap modal title
...
function delete_jenisproduk(id)
{
    save_method = 'delete';
    //$('#modal_form').removeData('bs.modal');
    var validator = $('#form').data('bootstrapValidator');
    ...
    $.ajax({
        ...
        success: function(data)
        {
            ...
            $('[name="id"]').val(data.id);
            $('[name="idjenisproduk"]').val(data.idjenisproduk).prop("readonly", true);
            $('[name="nmjenisproduk"]').val(data.nmjenisproduk).prop("readonly", true);
            $('[name="chold"]').val(data.chold.trim());
            ...
            //$('[name="dob"]').datepicker('update',data.dob);
            $('#modal_form').modal('show'); // show bootstrap modal when complete loaded
            $('.modal-title').text('Delete jenisproduk'); // Set title to Bootstrap modal title
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            if (validator.isValid()) {
                ...
                var validjenisproduk= $('[name="idjenisproduk"]').val();
                var valnmjenisproduk = $('[name="nmjenisproduk"]').val();
                ...
                var fillData = {
                    ...
                    'body': {
                        id: valID,
                        idjenisproduk: validjenisproduk,
                        nmjenisproduk: valnmjenisproduk,
                        // pbiaya: valpbiaya,
                        chold: valchold,
                        type: valtype
                    },
                ...
                $.ajax({
                    type: "POST",
                    url: HOST_URL + 'master/data/saveJenisProduk' + '',
                    dataType: 'json',
                    ...
                    success: function (datax) {
                        if (datax.status) {
                            Swal.fire({
                                ...
                                /* Read more about isConfirmed, isDenied below */
                                if (result.isConfirmed) {
                                    // window.jenisproduk.replace(HOST_URL + 'trans/referensisdm')
                                    reload_table();
...
// ID POSTION
var defaultInitialGroup = $('[name="idjenisproduk"]').val();
