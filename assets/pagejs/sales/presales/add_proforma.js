...
function documentReadable(){
    ...
    $.ajax({
        type: 'GET',
        url: HOST_URL + 'sales/presales/showing_proformatemp',
        data: { docno: docno },
        dataType: 'json',
        success: function(json) {
            ...
            if(json.dataTables.items.length != 0){
                if (json.dataTables.items[0].status.trim() === 'E') {
                    ...
                    let parts = docnoData.split('/'); // ["JTS", "PROF", "0001", "IX", "2025"]
                    ...
                    $.ajax({
                        type: 'GET',
                        url: HOST_URL + 'api/globalmodule/list_bank_sales?var=' + json.dataTables.items[0].bank,
                        dataType: 'json',
                    }).then(function (datax) {
                        var option = new Option(datax.items[0].nmbank, datax.items[0].id, true, true);
                        $('[name="bank"]').append(option).trigger('change').trigger({
                            type: 'select2:select',
                            params: { data: datax }
                        });
                    });
                    ...
                    $('[name="grosssales"]').val(convertToDbNumber(safeTrim(json.dataTables.items[0].grosssales))).prop('readonly', true);
                    $('[name="downpayment"]').val(convertToDbNumber(safeTrim(json.dataTables.items[0].downpayment))).prop('readonly', false);
                    $('[name="netsales"]').val(convertToDbNumber(safeTrim(json.dataTables.items[0].netsales))).prop('readonly', true);
                    $('[name="taxbasis"]').val(convertToDbNumber(safeTrim(json.dataTables.items[0].taxbasis))).prop('readonly', true);
                    $('[name="vat"]').val(convertToDbNumber(safeTrim(json.dataTables.items[0].vat))).prop('readonly', true);
                    $('[name="pph22"]').val(convertToDbNumber(safeTrim(json.dataTables.items[0].pph22))).prop('readonly', false);
                    $('[name="ttlprice"]').val(convertToDbNumber(safeTrim(json.dataTables.items[0].ttlprice))).prop('readonly', true);
...
function generateDocNo() {
        ...
        if (rolejob && jnsinvoice) {
            $.ajax({
                url: HOST_URL + '/sales/presales/getRolePOProforma',
                method: 'GET',
                data: {
                    jnsinvoice:  $('#jnsinvoice').val(),
                    rolejob: rolejob,
                    codemenu: 'I.S.A.2'
                },
                dataType: 'json',
                success: function (res) {
                    if (res.success) {
                        ...
                        // $('#infix2').val(res.infix);       // INFIX
                        // $('#infix3').val(res.infix);       // INFIX
                        $('#sufix').val(res.suffix);           // SUFIX

                        // parse infix YYMM -> year, month
                        var sufix = (res.suffix || '').toString(); // contoh: "IX/2025"
                        if (sufix.includes('/')) {
                            $('#docdate').prop('disabled', false);
                            var parts = sufix.split('/');
                            var romanMonth = parts[0]; // IX
                            var year = parseInt(parts[1], 10);

                            // konversi romawi ke angka bulan
                            var romanMap = {
                                'I':1, 'II':2, 'III':3, 'IV':4, 'V':5, 'VI':6,
                                'VII':7, 'VIII':8, 'IX':9, 'X':10, 'XI':11, 'XII':12
                            };
                            var month = romanMap[romanMonth];

                            if (month) {
                                var startDate = moment([year, month-1, 1]);       // bulan romawi -> index 0-based
                                var endDate   = moment(startDate).endOf('month'); // akhir bulan
                                ...
                                // isi default ke tanggal awal bulan
...
function tableProformaDtl(){
    ...
    var table = $('#t_proforma_dtl').DataTable({
        ...
        "iDisplayLength": -1,
        "ajax": {
            "url": HOST_URL + 'sales/presales/list_t_proforma_dtl',
            "type": "POST",
            "data": function(data) {
    ...
    $('#delete_item').on('click', function(e){
        ...
        const $button = $(this);
        if(isEditing) {
            reloadtableProformaDtl();  // Reload table to restore initial data
                ...
                $.each(rows_selected, function(index, rowId){
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            $.ajax({
                                url: HOST_URL + 'sales/presales/deleteProformaDtl',
                                type: "POST",
                                ...
                                dataFilter: function (data) {
                                    ...
                                    if (json.status) //if success close modal and reload ajax table
                                    {
                                        ...
                                        $('#t_proforma_dtl').DataTable().ajax.reload(function () {
...
function reloadtableProformaDtl()
{
    var table = $('#t_proforma_dtl');
...
$('#t_proforma_dtl').on('draw.dt', function () {
...
$("#update_item").click(function (e) {
    ...
    if (!isEditing) {
      ...
      // Enable kolom type=number khusus di t_proforma_dtl
        $("input[id^='qty_'], input[id^='price_'], input[id^='amount']")
            .prop("disabled", false)
            .css("background-color", "#ffffff");

        // Enable kolom type=text khusus di t_proforma_dtl
        ...
        let updatedData = [];
        $("#t_proforma_dtl tbody tr").each(function () {
        ...
        $.ajax({
            url: "update_detail_proforma", // Ganti dengan endpoint update di backend
            type: "POST",
            data: { 
                updates: updatedData,
        ...
        $("input[id^='qty_'], input[id^='price_'], input[id^='amount_'], input[id^='description_']")
            .prop("disabled", true)
...
$("#cust").select2({
    ...
    // minimumInputLength: 1,
    templateResult: formatCust, // omitted for brevity, see the source of this page
    templateSelection: formatCustSelection // omitted for brevity, see the source of this page
...
function finalEntryProforma() {
    ...
    // Ambil field dari card Informasi Pembayaran
    let bank         = $("#bank").val();
    let nmbank       = $("#nmbank").val();
    let alamatbank       = $("#alamatbank").val();
    let accno       = $("#accno").val();
    ...
    if (!bank) {
        Swal.fire({
            icon: 'warning',
            title: 'Perhatian',
            text: 'Bank harus dipilih terlebih dahulu!'
        });
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: HOST_URL + 'sales/presales/finalEntryProforma',
                type: "POST",
                data: data,
                dataType: "json",
                success: function(res) {
...
function insertNewProforma() {
    ...
    $.ajax({
        type: "POST",
        url: HOST_URL + 'sales/presales/insertNewProforma',
        dataType: 'json',
...
$(document).on('input', '.form-control', function () {
    ...
    if(id){
        let qty = parseFloat($('#qty_' + id).val().replace(/,/g, '')) || 0;
        let price = parseFloat($('#price_' + id).val().replace(/,/g, '')) || 0;
        let amount = qty * price;    
        $('#amount_' + id).val(amount.toLocaleString('en-US', {
...
function calculateSummary() {
   let gross = 0;
    ...
    $("#t_proforma_dtl tbody tr").each(function() {
        let val = $(this).find("input.amount").val() || "0";
        // konversi dulu dengan convertToDbNumber
        let dbNumber = convertToDbNumber(val);
        gross += dbNumber;
    });

    // Ambil DP
    let dp = convertToDbNumber(($("#downpayment").val())) || 0;
    ...
    // Update ke field
    $("#grosssales").val(formatUI(gross));
    $("#netsales").val(formatUI(net));
    $("#taxbasis").val(formatUI(taxbasis));
    $("#vat").val(formatUI(vat));
    $("#pph22").val(formatUI(pph22));
    $("#ttlprice").val(formatUI(ttl));
}
