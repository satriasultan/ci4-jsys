...
function documentReadable(){
    ...
    $.ajax({
        type: 'GET',
        url: HOST_URL + 'sales/presales/showing_proformatrx',
        data: { docno: docno },
        dataType: 'json',
        success: function(json) {
            ...
            if(json.dataTables.items.length != 0){
                // if (json.dataTables.items[0].status.trim() === 'E') {
                    ...
                    $('[name="grosssales"]').val(json.dataTables.items[0].grosssales.trim()).prop('readonly', true);
                    $('[name="downpayment"]').val(json.dataTables.items[0].downpayment.trim()).prop('readonly', true);
                    $('[name="netsales"]').val(json.dataTables.items[0].netsales.trim()).prop('readonly', true);
                    $('[name="taxbasis"]').val(json.dataTables.items[0].taxbasis.trim()).prop('readonly', true);
                    $('[name="vat"]').val(json.dataTables.items[0].vat.trim()).prop('readonly', true);
                    $('[name="pph22"]').val(json.dataTables.items[0].pph22.trim()).prop('readonly', true);
                    $('[name="ttlprice"]').val(json.dataTables.items[0].ttlprice.trim()).prop('readonly', true);

                    $('[name="bank"]').val(json.dataTables.items[0].bank).prop('disabled', true);
...
function tableProformaDtl(){
    ...
    var table = $('#t_proforma_dtl').DataTable({
        ...
        "iDisplayLength": -1,
        "ajax": {
            "url": HOST_URL + 'sales/presales/list_t_proforma_dtltrx',
            "type": "POST",
            "data": function(data) {
