...
function documentReadable(){
    ...
    $.ajax({
        type: 'GET',
        url: HOST_URL + 'sales/postsales/showing_salesordertrx',
        data: { docno: docno },
        dataType: 'json',
        success: function(json) {
            ...
            if(json.dataTables.items.length != 0){
                // if (json.dataTables.items[0].status.trim() === 'E') {
                    ...
                    $.ajax({
                        type: 'GET',
                        url: HOST_URL + 'api/globalmodule/list_currency?var=' + json.dataTables.items[0].currency,
                        dataType: 'json',
                    }).then(function (datax) {
                        var option = new Option(datax.items[0].currname, datax.items[0].currcode, true, true);
                        $('[name="currency"]').append(option).trigger('change').trigger({
                            type: 'select2:select',
                            params: { data: datax }
                        });
                    });
                    ...
                    $('[name="currency"]').val(json.dataTables.items[0].currency).prop('disabled', true);
                    $('[name="exchangerate"]').val(dbToIndoFormat(json.dataTables.items[0].exchangerate)).prop('readonly', true);
                    $('[name="enduser"]').val(json.dataTables.items[0].enduser).prop('readonly', true);
                    $('[name="trader"]').val(json.dataTables.items[0].trader).prop('readonly', true);
                    $('[name="pocust"]').val(json.dataTables.items[0].pocust).prop('readonly', true);
                    
                    // $('[name="facrisk"]').val(json.dataTables.items[0].facrisk.trim()).prop('readonly', true);
                    // $('[name="shipper"]').val(json.dataTables.items[0].shipper.trim()).prop('readonly', true);
                    // $('[name="consignee"]').val(json.dataTables.items[0].consignee.trim()).prop('readonly', true);
                    // $('[name="shippingmark"]').val(json.dataTables.items[0].shippingmark.trim()).prop('readonly', true);
                    // $('[name="notifyparty"]').val(json.dataTables.items[0].notifyparty.trim()).prop('readonly', true);

                    $('[name="grosssales"]').val(dbToIndoFormat(safeTrim(json.dataTables.items[0].grosssales))).prop('readonly', true);
                    $('[name="downpayment"]').val(dbToIndoFormat(safeTrim(json.dataTables.items[0].downpayment))).prop('readonly', true);
                    $('[name="netsales"]').val(dbToIndoFormat(safeTrim(json.dataTables.items[0].netsales))).prop('readonly', true);
                    $('[name="taxbasis"]').val(dbToIndoFormat(safeTrim(json.dataTables.items[0].taxbasis))).prop('readonly', true);
                    $('[name="vat"]').val(dbToIndoFormat(safeTrim(json.dataTables.items[0].vat))).prop('readonly', true);
                    $('[name="pph22"]').val(dbToIndoFormat(safeTrim(json.dataTables.items[0].pph22))).prop('readonly', true);
                    $('[name="ttlprice"]').val(dbToIndoFormat(safeTrim(json.dataTables.items[0].ttlprice))).prop('readonly', true);
                    ...
                    $('[name="paymentmethod"]').val((json.dataTables.items[0].paymentmethod)).prop('readonly', true);
                    $('[name="remark"]').val((json.dataTables.items[0].remark)).prop('readonly', true);
...
$('#rolejob').on('change', function () {
        ...
        if (rolejob) {
            $.ajax({
                url: HOST_URL + '/sales/postsales/getRolePO',
                method: 'GET',
                data: {
                    rolejob: rolejob,
                    codemenu: 'I.S.A.2'
...
function tableSalesOrderDtl(){
    ...
    var table = $('#t_salesorder_dtl').DataTable({
        ...
        "iDisplayLength": -1,
        "ajax": {
            "url": HOST_URL + 'sales/postsales/list_t_salesorder_dtltrx',
            "type": "POST",
            "data": function(data) {
    ...
    $('#delete_item').on('click', function(e){
        ...
        const $button = $(this);
        if(isEditing) {
            reloadtableSalesOrderDtl();  // Reload table to restore initial data
                ...
                $.each(rows_selected, function(index, rowId){
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            $.ajax({
                                url: HOST_URL + 'sales/postsales/deleteOfferingDtl',
                                type: "POST",
                                ...
                                dataFilter: function (data) {
                                    ...
                                    if (json.status) //if success close modal and reload ajax table
                                    {
                                        ...
                                        $('#t_salesorder_dtl').DataTable().ajax.reload(function () {
...
function reloadtableSalesOrderDtl()
{
    var table = $('#t_salesorder_dtl');
...
$("#update_item").click(function (e) {
    ...
    if (!isEditing) {
      ...
      // Enable kolom type=number khusus di t_salesorder_dtl
        $("input[id^='qty_'], input[id^='price_'], input[id^='amount']")
            .prop("disabled", false)
            .css("background-color", "#ffffff");

        // Enable kolom type=text khusus di t_salesorder_dtl
        ...
        let updatedData = [];
        $("#t_salesorder_dtl tbody tr").each(function () {
        ...
        $.ajax({
            url: "update_detail_salesorder", // Ganti dengan endpoint update di backend
            type: "POST",
            data: { 
                updates: updatedData,
