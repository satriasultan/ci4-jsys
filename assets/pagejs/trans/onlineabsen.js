...
var table;
"use strict";
function tableOnlineabsen(){
    // var lg = languageDatatable;
    var initTable = function () {
        var table = $('#tonlineabsen');
        table.DataTable({
            ...
            "ajax": {
                "url": HOST_URL + ('trans/shareloc/list_absen_online'),
                "type": "POST",
                "data": function(data) {
                    data.tglrange = $('#daterangefilter').val();
                    data.dept = $('#id_dept_filter').val();
                    data.nik = $('#nik_filter').val();
...
$("#nik_filter").select2({
    placeholder: "Ketik Dan Pilih Nik/Nama Karyawan",
    allowClear: true,
    ajax: {
...
{           //$('[name="dob"]').datepicker('update',data.dob);
            $('#modal_show_image').modal('show'); // show bootstrap modal when complete loaded
            $('.modal-title').text('Show Image'); // Set title to Bootstrap modal title
            photo.setAttribute('src', id);
            console.log(id);
}
/* TMP CHECKINOUT USER */
function tableCheckinout(){
    // var lg = languageDatatable;
    var initTable = function () {
        var table = $('#tcheckinout');
        table.DataTable({
            "processing": true, //Feature control the processing indicator.
            "serverSide": true, //Feature control DataTables' server-side processing mode.
            "order": [], //Initial no order.
            ...
            "ajax": {
                "url": HOST_URL + ('trans/shareloc/list_report_checkinout'),
                "type": "POST",
                "data": function(data) {
                    data.tglrange = $('#daterangefilter').val();
                    data.dept = $('#id_dept_filter').val();
...
function reload_table_checkinout()
{
    var table = $('#tcheckinout');
    table.DataTable().ajax.reload(); //reload datatable ajax
}
$('#btn-checkinout-filter').click(function(){ //button filter event click
    var table = $('#tcheckinout');
    table.DataTable().ajax.reload(); //reload datatable ajax
    $('#filter-checkinout').modal('hide');
});
$('#btn-checkinout-reset').click(function(){ //button reset event click
    $('#form-filter-checkinout')[0].reset();
    var table = $('#tcheckinout');
    table.DataTable().ajax.reload(); //reload datatable ajax
    $('#filter-checkinout').modal('hide');
});
...
$(document).ready(function() {
    ...
    $("#daterangeDownload").daterangepicker({
        //singleDatePicker: true,
        //showDropdowns: true
    });
    $("#daterangefilter").val('');
