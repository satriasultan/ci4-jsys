...
function tableReadable(){
    ...
    $('[name="nik"]').change(function () {
        ...
        $.ajax({
            type: 'GET',
            url: HOST_URL + 'trans/abscut/showDetailNik' + '?nik=' + $(this).val().trim(),
            dataType: 'json',
...
function documentReadable(){
    $.ajax({
        type: 'GET',
        url: HOST_URL + 'trans/abscut/showDetailAbscut' + '?docno=' + $('[name="docno"]').val(),
        dataType: 'json',
        dataFilter: function(data) {
            var json = jQuery.parseJSON(data);
            json.status = json.dataTables.status;
            ...
            $.ajax({
                type: 'GET',
                url: HOST_URL + 'trans/abscut/list_mtypecuti' + '?var=' + json.dataTables.items[0].idtypecuti,
                dataType: 'json',
                delay: 250,
            }).then(function (datax) {
...
$(".select2_kary").select2({
    ...
    allowClear: true,
    ajax: {
        url: HOST_URL + 'trans/abscut/list_karyawan',
        type: 'POST',
        dataType: 'json',
        delay: 250,
...
function editDocument(e){
    Swal.fire({
        ...
        if (result.isConfirmed) {
            window.location.replace(HOST_URL + 'trans/abscut/edit' + '/?docno=' + e)
...
$("#idtypecuti").select2({
    ...
    allowClear: true,
    ajax: {
        url: HOST_URL + 'trans/abscut/list_mtypecuti',
        type: 'POST',
        dataType: 'json',
        delay: 250,
        data: function(params) {
...
function detailDocument(e){
    Swal.fire({
        ...
        /* Read more about isConfirmed, isDenied below */
        if (result.isConfirmed) {
            window.location.replace(HOST_URL + 'trans/abscut/detail' + '/?docno=' + e)
        } else if (result.isDenied) {
            return false;
        }
    })
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            if (validator.isValid()) {
                ...
                $.ajax({
                    url: HOST_URL + 'trans/abscut/saveAbscut',
                    type: "POST",
                        ...
                        if (json.status) //if success close modal and reload ajax table
                        {
                            Swal.fire({
                                ...
                                if (result.isConfirmed) {
                                    ...
                                    window.location.replace(HOST_URL + 'trans/abscut')
                                }
                                // } else if (result.isDenied) {
...
function tableMkriteria(){
    ...
    var initTable = function () {
        ...
        table.DataTable({
            ...
            "ajax": {
                "url": HOST_URL + ('trans/abscut/list_mkriteria'),
                "type": "POST",
                "data": function(data) {
...
function update_mkriteria(id)
{
    ...
    $.ajax({
        url: HOST_URL + 'trans/abscut/showing_data_kriteria' + '/' + id,
        type: "GET",
        dataType: "JSON",
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            if (validator.isValid()) {
                ...
                $.ajax({
                    type: "POST",
                    url: HOST_URL + 'trans/abscut/saveEntryKriteria' + '',
                    dataType: 'json',
...
$(document).ready(function() {
    tableMkriteria();
    $("#tAbscutResume").dataTable({ "pageLength": 25,"lengthMenu": [[10, 25, 50, -1], [10, 25, 50, "All"]] });
