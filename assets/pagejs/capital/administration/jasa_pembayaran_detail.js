...
function tableCapitalPengajuan(){
    ...
    var initTable = function () {
        ...
        table.DataTable({
            ...
            "ajax": {
                "url": HOST_URL + 'capital/administration/list_jasa_pembayaran_detail',
                "type": "POST",
                "data": function(data) {
                    //data.idrev_x = $('[name="idrev_x"]');
                    data.idrev_x = $('[name="idrev_x"]').val();
                },
...
function add_pembayaran_detail(id)
{
    save_method = 'add';
    ...
    $.ajax({
        url: HOST_URL + 'capital/administration/showing_jasa_pembayaran' + '/' + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            $('[name="type"]').val('INPUT');
            $('[name="id"]').val(data.id);
            $('[name="idrev"]').val(data.id);
            $('[name="nmpembayaran"]').val(data.nmpembayaran);
            $('[name="nmsupplier"]').val(data.nmsupplier);
            $('[name="renewal"]').val(data.renewal.trim()).trigger('change');
            $('[name="ppn"]').val(data.ppn.trim()).trigger('change');
            $('[name="jnsppn"]').val(data.jnsppn.trim()).trigger('change');
            $('[name="ttlnetto"]').val(data.ttlnetto1.trim());
            $('[name="duedate_next"]').val(data.duedate_next);
            $('[name="duedate_in"]').val(data.duedate_in);
            $('[name="invoice_date"]').val('');


            $('[name="nmpembayaran"]').prop("readonly", true);
            $('[name="nmsupplier"]').prop("readonly", true);
            $('[name="renewal"]').prop("readonly", true);
            $('[name="duedate_in"]').prop("readonly", true);
            $('[name="duedate_next"]').prop("readonly", true);
            $('[name="duedate_renewal"]').prop("readonly", false);

            $('#btnSave').removeClass("btn-primary").addClass("btn-info").text('Simpan');
            //$('[name="dob"]').datepicker('update',data.dob);
            $('#modal_form').modal('show'); // show bootstrap modal when complete loaded
            $('.modal-title').text('Simpan History Pembayaran'); // Set title to Bootstrap modal title
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            if (validator.isValid()) {
                ...
                $.ajax({
                    url: HOST_URL + 'capital/administration/save_jasa_pembayaran_detail',
                    type: "POST",
                        ...
                        if (json.status) //if success close modal and reload ajax table
                        {
                            Swal.fire({
                                ...
                                if (result.isConfirmed) {
                                    ...
                                    window.location.replace(HOST_URL + 'capital/administration/detail_jasa_pembayaran?q=' +  $('[name="idrev"]').val())
...
function delete_pembayaran_detail(id)
{
    ...
    Swal.fire({
        title: 'Peringatan..!!!',
        text: 'Hapus History Pembayaran...?' + id,
        backdrop: true,
        allowOutsideClick: false,
        showConfirmButton: true,
        showDenyButton: true,
        ...
        /* Read more about isConfirmed, isDenied below */
        if (result.isConfirmed) {
            window.location.replace(HOST_URL + 'capital/administration/delete_detail_pembayaran/' +  id)
        }
    });
}
