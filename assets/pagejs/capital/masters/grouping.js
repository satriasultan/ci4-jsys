...
function tableMGroup(){
    ...
    var initTable = function () {
        ...
        table.DataTable({
            ...
            "ajax": {
                "url": HOST_URL + 'capital/masters/list_mgroup',
                "type": "POST",
                "data": function(data) {
...
{
    save_method = 'add';
    var validator = $('#formMgroup').data('bootstrapValidator');
    validator.resetForm();
    $('.inform').removeAttr("disabled").removeAttr("readonly").removeAttr("text");
    $('#formMgroup')[0].reset(); // reset form on modals
    //$('.form-group').removeClass('has-error').removeClass('has-success'); // clear error class
    $('.help-block').empty(); // clear error string
    $('#modal_form').modal('show'); // show bootstrap modal
    $('.modal-title').text('Input Kategori Baru'); // Set Title to Bootstrap modal title
    $('[name="type"]').val('INPUT');
    $('[name="id"]').val();
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            if (validator.isValid()) {
                var valtype= $('[name="typeMgroup"]').val();
                var valid= $('[name="idMgroup"]').val();
                var validgroup= $('[name="idgroupMgroup"]').val();
                var valintype= 'GROUP';
                var valnmgroup= $('[name="nmgroupMgroup"]').val();
                var valgrouptype= $('[name="grouptypeMgroup"]').val();
                var valdescription= $('[name="descriptionMgroup"]').val();
                var valchold= $('[name="choldMgroup"]').val();
                var fillData = {
                    'success': true,
                ...
                $.ajax({
                    type: "POST",
                    url: HOST_URL + 'capital/masters/saveGrouping' + '',
                    dataType: 'json',
