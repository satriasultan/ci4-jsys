...
{
    save_method = 'add';
    var validator = $('#formPengajuanBudget').data('bootstrapValidator');
    validator.resetForm();
    $('.inform').removeAttr("disabled").removeAttr("readonly").removeAttr("text");
    $('#formPengajuanBudget')[0].reset(); // reset form on modals
...
$('#formPengajuanBudget').bootstrapValidator({
            ...
            if (validator.isValid()) {
                ...
                var fillData = {
                    ...
                    'body': {
                        ...
                        docdate: valdocdate,
                        daterealization: valdaterealization,
                        idcostcenter: validcostcenter,
                        spec: valspec,
                        estvendor: valestvendor,
                        btype: valbtype,
                        bsubtype: valbsubtype,
                        acctype: valacctype,
                        docno: valdocno,
                ...
                $.ajax({
                    type: "POST",
                    url: HOST_URL + 'capital/administration/save_capital_pengajuan_budget' + '',
                    dataType: 'json',
...
$("#kdlvl").on("change", function () {
    ...
    if (!idsubgroup && id){
        ...
        //$('#formPengajuanBudget').bootstrapValidator('updateStatus', 'idsubgroup', 'NOT_VALIDATED').bootstrapValidator('validateField', 'idsubgroup');
    } else {
        //Ajax Load data from ajax
        $.ajax({
...
var defAccounting = 'ACC';
$("#acctype").select2({
    placeholder: "Type/Pilih Kategori Accounting",
    allowClear: true,
    ajax: {
        url: HOST_URL + 'api/globalmodule/list_mgroup',
        type: 'POST',
        ...
        data: function(params) {
            return {
                ...
                _start_: 1,
                _perpage_: 2,
                _paramglobal_: defAccounting,
                _parameterx_: defAccounting,
            };
        },
        processResults: function(data, params) {
            // parse the results into the format expected by Select2
...
var defBtype = 'BGT';
$("#btype").select2({
    placeholder: "Type/Pilih Kategori Budget",
    allowClear: true,
    ajax: {
        url: HOST_URL + 'api/globalmodule/list_mgroup',
        type: 'POST',
        ...
        data: function(params) {
            return {
                ...
                _start_: 1,
                _perpage_: 2,
                _paramglobal_: defBtype,
                _parameterx_: defBtype,
            };
        },
        processResults: function(data, params) {
            // parse the results into the format expected by Select2
    ...
    $("#bsubtype option[value]").remove();
    var newOptions = []; // the result of your JSON request
    $("#bsubtype").val(null).trigger('change');
    //console.log($("#newdept").val());
});
var bsubtypeInitial = 'BGT';
$("#bsubtype").select2({
    placeholder: "Type/Pilih Sub Kategori",
    allowClear: true,
    ajax: {
        ...
        data: function(params) {
            return {
                ...
                _start_: 1,
                _perpage_: 2,
                _paramglobal_: bsubtypeInitial,
                _parameterx_: bsubtypeInitial,
                _var_: $("#btype").val(),
            };
        },
        processResults: function(data, params) {
