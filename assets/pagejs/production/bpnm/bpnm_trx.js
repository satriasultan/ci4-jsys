...
function documentReadable() {
    ...
    $.getJSON(HOST_URL + 'production/trans/showing_trx_bpnm_mst', {docno: docno})
...
/* TABLE PP DETAIL */
function tabletrxBPNM() {
    /* Tabel PP Detail */
    var initTable = function () {
        var table = $('#trx_bpnmdtl');
        table.DataTable({
            "processing": true, //Feature control the processing indicator.
            "serverSide": true, //Feature control DataTables' server-side processing mode.
            "language": languageDatatable(),
            ...
            "iDisplayLength": -1,
            "ajax": {
                "url": HOST_URL + 'production/trans/list_trx_bpnm_dtl',
                "type": "POST",
                "data": function (data) {
...
// CHECK ALL
$('#trx_bpnmdtl thead').on('change', '#checkAll', function () {
    const checked = this.checked;

    $('#trx_bpnmdtl tbody .row-check').prop('checked', checked);
});

// JIKA SALAH SATU ROW UNCHECK → CHECKALL MATI
$('#trx_bpnmdtl tbody').on('change', '.row-check', function () {
    const total = $('#trx_bpnmdtl tbody .row-check').length;
    const checked = $('#trx_bpnmdtl tbody .row-check:checked').length;

    $('#checkAll').prop('checked', total === checked);
});

$('#trx_bpnmdtl').on('draw.dt', function () {
