...
function table_bpnm() {
    ...
    var initTable = function () {
        ...
        table.DataTable({
            ...
            "ajax": {
                "url": HOST_URL + 'production/trans/list_bpnm_mst',
                "type": "POST",
                "data": function (data) {
...
$('#cabang').on('change', function () {
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            generateDocNumber('BNM', res.infix, currentKodeSuffix + '0001');
...
$('#prefix').on('change', function () {
    ...
    $.ajax({
        url: HOST_URL + '/production/trans/getNextSuffix_bpnm_mst',
        method: 'GET',
        data: {
            prefix: prefix,
            infix: infix,
...
function documentReadable() {
    ...
    $.getJSON(HOST_URL + 'production/trans/showing_tmp_bpnm_mst', {docno: docno})
...
function btnInputDetailMaterial() {
    ...
    // title modal
    $('#modalDtlBPNMTitle').text('Tambah Detail Material');
    resetBPNMModal();
...
/*    // destroy jika sudah pernah init
    ...
    // show modal
    $('#modalDtlBPNM').modal('show');
}

function resetBPNMModal(){
...
function updateDetailBPNM(){
    ...
    $.ajax({
        url: HOST_URL + 'production/trans/get_bpnm_dtl',
        type: 'GET',
        data: { id: id },
        dataType: 'json',
        success: function(res){
            if(res.status){
                ...
                setJtsValue('[name="harga"]', convertToDbNumber(res.items[0].harga));
                setJtsValue('[name="nilai"]', convertToDbNumber(res.items[0].nilai));
                ...
                $('#modalDtlBPNMTitle').text('Update Detail Material');
                $('#modalDtlBPNM').modal('show');
...
function btnDeleteDetailBPNM(){
    ...
    Swal.fire({
        ...
        $.ajax({
            url: HOST_URL + 'production/trans/delete_bpnm_detail',
            type: 'POST',
            data: { ids: ids },
            dataType: 'json',
            success: function(res){
...
function saveBPNMFromWO(oldWoeno)
{
    ...
    $.ajax({
        url: HOST_URL + 'production/trans/save_bpnm_from_wo',
        type: 'POST',
        processData:false,
        contentType:false,
        dataType:'json',
...
function saveBPNMDetail(formId) {
        ...
        $.ajax({
            url: HOST_URL + 'production/trans/save_bpnm_mst',
            type: 'POST',
            data: formData,
            dataType: 'json',
            processData: false,
            ...
            success: function (res) {
                ...
                if (res.success) {
                    ...
                    // if(detailType === 'MATERIAL'){
                    $('#modalDtlBPNM').modal('hide');
                    reload_bpnm_dtl();
                    $('#formBPNMMaterialDtl')[0].reset();
                    // } else if(detailType === 'COST'){
                    //     $('#modalDtlBomCost').modal('hide');
                    //     reload_bpnm_cost_dtl();
                    //     $('#formBPNMCostDtl')[0].reset();
                    // } else if(detailType === 'WIP'){
                    //     $('#modalDtlBomWip').modal('hide');
                    //     reload_bpnm_wip_dtl();
                    //     $('#formBPNMWipDtl')[0].reset();
                    // }
                    documentReadable()
...
/* TABLE PP DETAIL */
function tabletmpBPNM() {
    /* Tabel PP Detail */
    var initTable = function () {
        var table = $('#tmp_bpnmdtl');
        table.DataTable({
            "processing": true, //Feature control the processing indicator.
            "serverSide": true, //Feature control DataTables' server-side processing mode.
            "language": languageDatatable(),
            ...
            "iDisplayLength": -1,
            "ajax": {
                "url": HOST_URL + 'production/trans/list_tmp_bpnm_dtl',
                "type": "POST",
                "data": function (data) {
...
// CHECK ALL
$('#tmp_bpnmdtl thead').on('change', '#checkAll', function () {
    const checked = this.checked;

    $('#tmp_bpnmdtl tbody .row-check').prop('checked', checked);
});

// JIKA SALAH SATU ROW UNCHECK → CHECKALL MATI
$('#tmp_bpnmdtl tbody').on('change', '.row-check', function () {
    const total = $('#tmp_bpnmdtl tbody .row-check').length;
    const checked = $('#tmp_bpnmdtl tbody .row-check:checked').length;

    $('#checkAll').prop('checked', total === checked);
});

$('#tmp_bpnmdtl').on('draw.dt', function () {
