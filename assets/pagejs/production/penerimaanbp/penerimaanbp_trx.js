...
function documentReadable() {
    ...
    $.getJSON(HOST_URL + 'production/trans/showing_trx_penerimaanbp_mst', {docno: docno})
...
/* TABLE PP DETAIL */
function tabletrxPBP() {
    /* Tabel PP Detail */
    var initTable = function () {
        var table = $('#trx_penerimaanbpdtl');
        table.DataTable({
            "processing": true, //Feature control the processing indicator.
            "serverSide": true, //Feature control DataTables' server-side processing mode.
            "language": languageDatatable(),
            ...
            "iDisplayLength": -1,
            "ajax": {
                "url": HOST_URL + 'production/trans/list_trx_penerimaanbp_dtl',
                "type": "POST",
                "data": function (data) {
...
// CHECK ALL
$('#trx_penerimaanbpdtl thead').on('change', '#checkAll', function () {
    const checked = this.checked;

    $('#trx_penerimaanbpdtl tbody .row-check').prop('checked', checked);
});

// JIKA SALAH SATU ROW UNCHECK → CHECKALL MATI
$('#trx_penerimaanbpdtl tbody').on('change', '.row-check', function () {
    const total = $('#trx_penerimaanbpdtl tbody .row-check').length;
    const checked = $('#trx_penerimaanbpdtl tbody .row-check:checked').length;

    $('#checkAll').prop('checked', total === checked);
});

$('#trx_penerimaanbpdtl').on('draw.dt', function () {
