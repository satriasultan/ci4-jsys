...
/* VIUW UTAMA*/
function table_penerimaanbp() {
    // var lg = languageDatatable;
    var initTable = function () {
        var table = $('#tpenerimaanbp');
        table.DataTable({
            ...
            "ajax": {
                "url": HOST_URL + 'production/trans/list_penerimaanbp_mst',
                "type": "POST",
                "data": function (data) {
...
$('#cabang').on('change', function () {
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            generateDocNumber('PRO', res.infix, currentKodeSuffix + '0001');
...
$('#prefix').on('change', function () {
    ...
    $.ajax({
        url: HOST_URL + '/production/trans/getNextSuffix_penerimaanbp_mst',
        method: 'GET',
        data: {
            prefix: prefix,
            infix: infix,
...
function documentReadable() {
    ...
    $.getJSON(HOST_URL + 'production/trans/showing_tmp_penerimaanbp_mst', {docno: docno})
        .done(function (response) {
            ...
            $('[name="batchno"]').val(item.batchno);
            $('[name="buildfor"]').prop('disabled', true);
...
function btnInputDetailMaterial() {
    ...
    // title modal
    $('#modalDtlPBPTitle').text('Tambah Detail Material');
    resetPBPModal();
...
/*    // destroy jika sudah pernah init
    ...
    $('#issub').prop('checked', true).trigger('change').prop('disabled',true);



    // show modal
    $('#modalDtlPBP').modal('show');
}

function resetPBPModal(){
...
function updateDetailPBP(){
    ...
    $.ajax({
        url: HOST_URL + 'production/trans/get_penerimaanbp_dtl',
        type: 'GET',
        data: { id: id },
        dataType: 'json',
        success: function(res){
            if(res.status){
                ...
                var isSub = res.items[0].issub.trim() == 'T';
                $('#issub').prop('checked', isSub).trigger('change').prop('disabled',true);
                ...
                if(res.items[0].issub.trim() == 'T'){
                    ...
                    var idbarang = res.items[0].idbarang.trim();
                    var wono = $('#wono').val();
                    ...
                    $.ajax({
                        ...
                        delay: 250,
                        data: {
                            var: idbarang,  // Kosong atau isi dengan ID barang
                            wono: wono,  // Kirim wono
                            docdate: docdate // Kirim docdate jika perlu
                        }
                    }).then(function (datax) {
                        // create the option and append to Select2
                        var option = new Option(datax.items[0].nmbarang, datax.items[0].idbarang, true, true);
                        $('[name="idbarang_bom"]').append(option).trigger('change')
    
                        // manually trigger the `select2:select` event
                        $('[name="idbarang_bom"]').trigger({
                            type: 'select2:select',
                            params: {
                                data: datax
                            }
                ...
                $('#modalDtlPBPTitle').text('Update Detail Material');
                $('#modalDtlPBP').modal('show');
...
function btnDeleteDetailPBP(){
    ...
    Swal.fire({
        ...
        $.ajax({
            url: HOST_URL + 'production/trans/delete_penerimaanbp_detail',
            type: 'POST',
            data: { ids: ids },
            dataType: 'json',
            success: function(res){
...
$("#idbarang").select2({
    ...
    // $('[name="nilai"]').val(data.newcost.trim()).prop("readonly", true);
    // $('[name="unit"]').val(data.unit.trim()).prop("readonly", true);
    setJtsValue('[name="harga"]', convertToDbNumber(data.newcost));
...
function savePBPFromWO(oldWoeno)
{
    ...
    $.ajax({
        url: HOST_URL + 'production/trans/save_penerimaanbp_from_wo',
        type: 'POST',
        processData:false,
        contentType:false,
        dataType:'json',
...
function savePBPDetail(formId) {
        ...
        formData.set('nilai',convertToDbNumber($('#nilai').val()));
        formData.set('harga',convertToDbNumber($('#harga').val()));
        ...
        $.ajax({
            url: HOST_URL + 'production/trans/save_penerimaanbp_mst',
            type: 'POST',
            data: formData,
            dataType: 'json',
            processData: false,
            ...
            success: function (res) {
                ...
                if (res.success) {
                    ...
                    // if(detailType === 'MATERIAL'){
                    $('#modalDtlPBP').modal('hide');
                    reload_penerimaanbp_dtl();
                    $('#formPBPMaterialDtl')[0].reset();
                    // } else if(detailType === 'COST'){
                    //     $('#modalDtlBomCost').modal('hide');
                    //     reload_penerimaanbp_cost_dtl();
                    //     $('#formPBPCostDtl')[0].reset();
                    // } else if(detailType === 'WIP'){
                    //     $('#modalDtlBomWip').modal('hide');
                    //     reload_penerimaanbp_wip_dtl();
                    //     $('#formPBPWipDtl')[0].reset();
                    // }
                    documentReadable()
...
/* TABLE PP DETAIL */
function tabletmpPBP() {
    /* Tabel PP Detail */
    var initTable = function () {
        var table = $('#tmp_penerimaanbpdtl');
        table.DataTable({
            "processing": true, //Feature control the processing indicator.
            "serverSide": true, //Feature control DataTables' server-side processing mode.
            "language": languageDatatable(),
            ...
            "iDisplayLength": -1,
            "ajax": {
                "url": HOST_URL + 'production/trans/list_tmp_penerimaanbp_dtl',
                "type": "POST",
                "data": function (data) {
...
$(document).on('copy', function (e) {
    ...
    var exportEl = $(node).closest('[data-export]');

    if (!exportEl.length) return;

    var value = exportEl.attr('data-export');
...
function reload_penerimaanbp_dtl() {
    var table = $('#tmp_penerimaanbpdtl');
    table.DataTable().ajax.reload(); //reload datatable ajax
}
...
// CHECK ALL
$('#tmp_penerimaanbpdtl thead').on('change', '#checkAll', function () {
    const checked = this.checked;

    $('#tmp_penerimaanbpdtl tbody .row-check').prop('checked', checked);
});

// JIKA SALAH SATU ROW UNCHECK → CHECKALL MATI
$('#tmp_penerimaanbpdtl tbody').on('change', '.row-check', function () {
    const total = $('#tmp_penerimaanbpdtl tbody .row-check').length;
    const checked = $('#tmp_penerimaanbpdtl tbody .row-check:checked').length;

    $('#checkAll').prop('checked', total === checked);
});

$('#tmp_penerimaanbpdtl').on('draw.dt', function () {
