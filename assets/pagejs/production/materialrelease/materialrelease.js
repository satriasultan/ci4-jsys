...
function documentReadable() {
    ...
    $.getJSON(HOST_URL + 'production/trans/showing_tmp_materialrelease_mst', {docno: docno})
        .done(function (response) {
            ...
            $('[name="wono"]').val(item.wono).prop("disabled", true);
...
/* TABLE PP DETAIL */
function tabletmpMaterialRelease() {
    /* Tabel PP Detail */
    var initTable = function () {
        var table = $('#tmp_materialreleasedtl');
        table.DataTable({
            "processing": true, //Feature control the processing indicator.
            "serverSide": true, //Feature control DataTables' server-side processing mode.
            "language": languageDatatable(),
            ...
            "iDisplayLength": -1,
            "ajax": {
                "url": HOST_URL + 'production/trans/list_tmp_materialrelease_dtl',
                "type": "POST",
                "data": function (data) {
...
// CHECK ALL
$('#tmp_materialreleasedtl thead').on('change', '#checkAll', function () {
    const checked = this.checked;

    $('#tmp_materialreleasedtl tbody .row-check').prop('checked', checked);
});

// JIKA SALAH SATU ROW UNCHECK → CHECKALL MATI
$('#tmp_materialreleasedtl tbody').on('change', '.row-check', function () {
    const total = $('#tmp_materialreleasedtl tbody .row-check').length;
    const checked = $('#tmp_materialreleasedtl tbody .row-check:checked').length;

    $('#checkAll').prop('checked', total === checked);
});

$('#tmp_materialreleasedtl').on('draw.dt', function () {
...
$(document).ready(function () {
    ...
    documentReadable();
    tabletmpMaterialRelease();
    // tabletmpMaterialReleaseCost();
    // tabletmpMaterialReleaseWip();
