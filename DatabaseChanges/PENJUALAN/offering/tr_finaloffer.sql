CREATE TRIGGER tr_offering_finalize
    AFTER UPDATE ON sc_tmp.offering
    FOR EACH ROW
...
BEGIN
    ...
    IF OLD.status = 'E' AND NEW.status = 'F' AND COALESCE(NEW.docnotmp, '') = '' THEN
        ...
        LOOP
            EXIT WHEN NOT EXISTS (SELECT 1 FROM sc_trx.offering WHERE docno = v_docno);
        ...
        SELECT
            idurut, v_docno, cust, address, docdate, phone, fax, up,
            description, brand, size, qty, pembayaran, pengiriman,
        ...
        SELECT
            idurut, v_docno, idbarang,nmbarang, unit, qty, price, exchange, usdmt, description,
            inputby, inputdate, status, updateby, updatedate
        FROM sc_tmp.offeringdtl
        WHERE docno = OLD.docno AND inputby = v_inputby;

        -- Hapus dari tmp
        DELETE FROM sc_tmp.offering WHERE docno = OLD.docno AND inputby = v_inputby AND idurut = v_idurut;
        DELETE FROM sc_tmp.offeringdtl WHERE docno = OLD.docno AND inputby = v_inputby;
    
    
    -- END IF;
    ELSEIF (OLD.STATUS = 'E' AND NEW.STATUS = 'F' AND COALESCE(NEW.docnotmp, '') != '') THEN
        DELETE FROM sc_trx.offering WHERE docno = NEW.docnotmp;
        DELETE FROM sc_trx.offeringdtl WHERE docno = NEW.docnotmp;

        -- Insert into offeringdtl with new columns
        INSERT INTO sc_trx.offeringdtl
        ( idurut, docno, idbarang,nmbarang, unit, qty, price, exchange, usdmt, description,
            inputby, inputdate, status, updateby, updatedate, docnotmp)
        SELECT  idurut,  NEW.docnotmp, idbarang,nmbarang, unit, qty, price, exchange, usdmt, description,
            inputby, inputdate, status, updateby, updatedate, docnotmp
        FROM sc_tmp.offeringdtl 
        ...
        SELECT 
        idurut, NEW.docnotmp, cust, address, docdate, phone, fax, up,
            description, brand, size, qty, pembayaran, pengiriman,
            expdate, ketentuan, status, inputby, inputdate,
            updateby, updatedate, printby, printdate, rolejob, docnotmp
        FROM sc_tmp.offering 
        ...
        DELETE FROM sc_tmp.offering WHERE docno = NEW.docno;
        DELETE FROM sc_tmp.offeringdtl WHERE docno = NEW.docno;

    ELSEIF (OLD.STATUS = 'E' AND NEW.STATUS = 'C') THEN
        IF NEW.printby IS NOT NULL AND NEW.printby <> '' AND NEW.printdate IS NOT NULL THEN
            UPDATE sc_trx.offering SET status = 'P' WHERE docno = NEW.docnotmp;
        ELSE
            UPDATE sc_trx.offering SET status = 'F' WHERE docno = NEW.docnotmp;
        END IF;
