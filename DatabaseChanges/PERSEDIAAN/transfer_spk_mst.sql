...
BEGIN
    IF OLD.status = 'E' AND NEW.status = 'F' AND COALESCE(NEW.docnotmp, '') = '' THEN
        ...
        -- INSERT DETAIL
        -- ===============================
        INSERT INTO sc_trx.transfer_spk_dtl 
		(idurut,docno,doctype,idbarang,nmbarang,unit,qty,description,status,val,valsum,inputby,inputdate,updateby,updatedate,iduniq,docnotmp)
        (SELECT idurut,v_docno,doctype,idbarang,nmbarang,unit,qty,description,'F' AS status,val,valsum,inputby,inputdate,updateby,updatedate,iduniq,docnotmp 
		FROM sc_tmp.transfer_spk_dtl WHERE rtrim(docno) = rtrim(OLD.docno) AND inputby = v_inputby);

        -- ===============================
        -- CLEANUP TMP
        -- ===============================
        DELETE FROM sc_tmp.transfer_spk_mst
        WHERE rtrim(docno) = rtrim(OLD.docno)
          AND inputby = v_inputby
          AND inputdate = v_inputdate;

        DELETE FROM sc_tmp.transfer_spk_dtl
        WHERE rtrim(docno) = rtrim(OLD.docno)
          AND inputby = v_inputby;
    ...
    ELSIF OLD.status = 'E' AND NEW.status = 'F' AND COALESCE(NEW.docnotmp, '') <> '' THEN
        ...
        DELETE FROM sc_trx.transfer_spk_mst WHERE docno = NEW.docnotmp;
        DELETE FROM sc_trx.transfer_spk_dtl WHERE docno = NEW.docnotmp;
        ...
        DELETE FROM sc_tmp.transfer_spk_mst WHERE rtrim(docno) = rtrim(NEW.docno);
    ...
    ELSEIF (OLD.STATUS = 'E' AND NEW.STATUS = 'C') THEN
        ...
        ELSE
            UPDATE sc_trx.transfer_spk_mst SET status = 'F' WHERE docno = NEW.docnotmp;
        END IF;

            
        DELETE FROM sc_tmp.transfer_spk_mst WHERE docno = NEW.docno;
        DELETE FROM sc_tmp.transfer_spk_dtl WHERE docno = NEW.docno;
...
BEGIN		
        ...
        -- INSERT DETAIL
        -- ===============================
        INSERT INTO sc_tmp.transfer_spk_dtl 
		(idurut,docno,doctype,idbarang,nmbarang,unit,qty,description,status,val,valsum,inputby,inputdate,updateby,updatedate,iduniq,docnotmp)
        (SELECT idurut,docno,doctype,idbarang,nmbarang,unit,qty,description,'F' AS status,val,valsum,inputby,inputdate,updateby,updatedate,iduniq,docno as docnotmp 
		FROM sc_trx.transfer_spk_dtl WHERE trim(docno) =  trim(new.docno));
