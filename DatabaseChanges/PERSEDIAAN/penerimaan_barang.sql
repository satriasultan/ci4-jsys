...
--drop table sc_tmp.penerimaan_brng_mst;
CREATE TABLE IF NOT EXISTS sc_tmp.penerimaan_brng_mst
(
    docno character(30) COLLATE pg_catalog."default" NOT NULL,
    doctype character(20) default 'penerimaan_brng' ,
    ...
    CONSTRAINT pk_tmp_penerimaan_brng_mst PRIMARY KEY (docno)
)

TABLESPACE pg_default;

ALTER TABLE IF EXISTS sc_tmp.penerimaan_brng_mst
    OWNER to postgres;


--drop table sc_trx.penerimaan_brng_mst;
...
BEGIN
    IF OLD.status = 'E' AND NEW.status = 'F' AND COALESCE(NEW.docnotmp, '') = '' THEN
        ...
        (SELECT v_docno,doctype,docdate,docref,cabang,cabang_sent,pemohon,estpakai,idlocation_from,idlocation_to,idlocation_transit,'F',description,inputby,inputdate,updateby,updatedate,printby,printdate,docnotmp FROM sc_tmp.pnm_brng_mst
		...
		FROM sc_tmp.pnm_brng_dtl WHERE rtrim(docno) = rtrim(OLD.docno) AND inputby = v_inputby);
    ...
    ELSIF OLD.status = 'E' AND NEW.status = 'F' AND COALESCE(NEW.docnotmp, '') <> '' THEN
        ...
        (SELECT new.docnotmp,doctype,docdate,docref,cabang,cabang_sent,pemohon,estpakai,idlocation_from,idlocation_to,idlocation_transit,'F',description,inputby,inputdate,updateby,updatedate,printby,printdate,docnotmp FROM sc_tmp.pnm_brng_mst
		...
		FROM sc_tmp.pnm_brng_dtl WHERE rtrim(docno) = rtrim(NEW.docno));
		

        DELETE FROM sc_tmp.pnm_brng_mst WHERE rtrim(docno) = rtrim(NEW.docno);
        DELETE FROM sc_tmp.pnm_brng_dtl WHERE rtrim(docno) = rtrim(NEW.docno);


    ELSEIF (OLD.STATUS = 'E' AND NEW.STATUS = 'C') THEN
        IF NEW.printby IS NOT NULL AND NEW.printby <> '' AND NEW.printdate IS NOT NULL THEN
            UPDATE sc_trx.pnm_brng_mst SET status = 'P' WHERE docno = NEW.docnotmp;
        ELSE
            UPDATE sc_trx.pnm_brng_mst SET status = 'F' WHERE docno = NEW.docnotmp;
        END IF;

            
        DELETE FROM sc_tmp.pnm_brng_mst WHERE docno = NEW.docno;
        DELETE FROM sc_tmp.pnm_brng_dtl WHERE docno = NEW.docno;
...
CREATE TRIGGER tr_tmp_pnm_brng_mst
    AFTER UPDATE ON sc_tmp.pnm_brng_mst
    FOR EACH ROW
...
BEGIN		
        ...
        (SELECT docno,doctype,docdate,docref,cabang,cabang_sent,pemohon,estpakai,idlocation_from,idlocation_to,idlocation_transit,'E',description,inputby,inputdate,updateby,updatedate,printby,printdate,docno as docnotmp FROM sc_trx.pnm_brng_mst
