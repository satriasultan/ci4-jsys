...
DROP TABLE IF EXISTS sc_tmp.umb
DROP TABLE IF EXISTS sc_trx.umb




CREATE TABLE IF NOT EXISTS sc_tmp.umb
(
    ...
    CONSTRAINT pk_tmp_umb PRIMARY KEY (docno)
)

TABLESPACE pg_default;

ALTER TABLE IF EXISTS sc_tmp.umb
    OWNER to postgres;
...
CREATE TABLE IF NOT EXISTS sc_trx.umb
(
    idurut serial NOT NULL,
    docno character(30) COLLATE pg_catalog."default" NOT NULL,
    docdate character(20) COLLATE pg_catalog."default",
    ...
    printdate timestamp without time zone,
    docnotmp character(30) COLLATE pg_catalog."default",
    CONSTRAINT pk_trx_umb PRIMARY KEY (docno)
)

TABLESPACE pg_default;

ALTER TABLE IF EXISTS sc_trx.umb
    OWNER to postgres;
...
-- FUNCTION: sc_tmp.tr_umb_finalize()

-- DROP FUNCTION IF EXISTS sc_tmp.tr_umb_finalize();
CREATE OR REPLACE FUNCTION sc_tmp.tr_umb_finalize()
RETURNS trigger
LANGUAGE plpgsql
AS $BODY$
DECLARE
...
BEGIN
    ...
    ELSIF OLD.status = 'E' AND NEW.status = 'F' AND COALESCE(NEW.docnotmp, '') <> '' THEN
        ...
        DELETE FROM sc_trx.umb WHERE docno = NEW.docnotmp;
        ...
        DELETE FROM sc_tmp.umb WHERE rtrim(docno) = rtrim(NEW.docno);
        
    ELSEIF (OLD.STATUS = 'E' AND NEW.STATUS = 'C') THEN
        IF NEW.printby IS NOT NULL AND NEW.printby <> '' AND NEW.printdate IS NOT NULL THEN
            UPDATE sc_trx.umb SET status = 'P' WHERE docno = NEW.docnotmp;
        ELSE
            UPDATE sc_trx.umb SET status = 'F' WHERE docno = NEW.docnotmp;
        END IF;

            
        DELETE FROM sc_tmp.umb WHERE docno = NEW.docno;
...
CREATE TRIGGER tr_umb_finalize
    AFTER UPDATE ON sc_tmp.umb
    FOR EACH ROW
    EXECUTE FUNCTION sc_tmp.tr_umb_finalize();
...
-- DROP FUNCTION IF EXISTS sc_trx.tr_umb();

CREATE OR REPLACE FUNCTION sc_trx.tr_umb()
    RETURNS trigger
    LANGUAGE 'plpgsql'
    COST 100
...
ALTER FUNCTION sc_trx.tr_umb()
    OWNER TO postgres;


    

-- FUNCTION: sc_trx.tr_umb()
-- Trigger: tr_umb

-- DROP TRIGGER IF EXISTS tr_umb ON sc_trx.umb;

CREATE OR REPLACE TRIGGER tr_umb
    AFTER UPDATE 
    ON sc_trx.umb
    FOR EACH ROW
    EXECUTE FUNCTION sc_trx.tr_umb();
