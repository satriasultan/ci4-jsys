...
DROP TABLE IF EXISTS sc_tmp.umt
DROP TABLE IF EXISTS sc_trx.umt




CREATE TABLE IF NOT EXISTS sc_tmp.umt
(
    ...
    CONSTRAINT pk_tmp_umt PRIMARY KEY (docno)
)

TABLESPACE pg_default;

ALTER TABLE IF EXISTS sc_tmp.umt
    OWNER to postgres;
...
CREATE TABLE IF NOT EXISTS sc_trx.umt
(
    idurut serial NOT NULL,
    docno character(30) COLLATE pg_catalog."default" NOT NULL,
    docdate character(20) COLLATE pg_catalog."default",
    ...
    printdate timestamp without time zone,
    docnotmp character(30) COLLATE pg_catalog."default",
    CONSTRAINT pk_trx_umt PRIMARY KEY (docno)
)

TABLESPACE pg_default;

ALTER TABLE IF EXISTS sc_trx.umt
    OWNER to postgres;
...
-- FUNCTION: sc_tmp.tr_umt_finalize()

-- DROP FUNCTION IF EXISTS sc_tmp.tr_umt_finalize();
CREATE OR REPLACE FUNCTION sc_tmp.tr_umt_finalize()
RETURNS trigger
LANGUAGE plpgsql
AS $BODY$
DECLARE
...
BEGIN
    ...
    ELSIF OLD.status = 'E' AND NEW.status = 'F' AND COALESCE(NEW.docnotmp, '') <> '' THEN
        ...
        DELETE FROM sc_trx.umt WHERE docno = NEW.docnotmp;
        ...
        DELETE FROM sc_tmp.umt WHERE rtrim(docno) = rtrim(NEW.docno);
        
    ELSEIF (OLD.STATUS = 'E' AND NEW.STATUS = 'C') THEN
        IF NEW.printby IS NOT NULL AND NEW.printby <> '' AND NEW.printdate IS NOT NULL THEN
            UPDATE sc_trx.umt SET status = 'P' WHERE docno = NEW.docnotmp;
        ELSE
            UPDATE sc_trx.umt SET status = 'F' WHERE docno = NEW.docnotmp;
        END IF;

            
        DELETE FROM sc_tmp.umt WHERE docno = NEW.docno;
...
CREATE TRIGGER tr_umt_finalize
    AFTER UPDATE ON sc_tmp.umt
    FOR EACH ROW
    EXECUTE FUNCTION sc_tmp.tr_umt_finalize();
...
-- DROP FUNCTION IF EXISTS sc_trx.tr_umt();

CREATE OR REPLACE FUNCTION sc_trx.tr_umt()
    RETURNS trigger
    LANGUAGE 'plpgsql'
    COST 100
...
ALTER FUNCTION sc_trx.tr_umt()
    OWNER TO postgres;


    

-- FUNCTION: sc_trx.tr_umt()
-- Trigger: tr_umt

-- DROP TRIGGER IF EXISTS tr_umt ON sc_trx.umt;

CREATE OR REPLACE TRIGGER tr_umt
    AFTER UPDATE 
    ON sc_trx.umt
    FOR EACH ROW
    EXECUTE FUNCTION sc_trx.tr_umt();
